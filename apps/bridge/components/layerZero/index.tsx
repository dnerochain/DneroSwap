...
import { Flex, Box } from '@dneroswap/uikit'
...
const LayerZero = ({ isWDnero }: { isWDnero?: boolean }) => {
  ...
  useEffect(() => {
    customElements.whenDefined('lz-bridge').then((Bridge: any) => {
      ...
      if (!Bridge.initialized) {
        ...
        const newTheme = {
          dark: createBasicTheme(DneroSwapTheme.dark),
          light: createBasicTheme(DneroSwapTheme.light),
        }
      ...
      if (isWDnero) {
        setTimeout(async () => {
          ...
          if (length !== null || length !== undefined) {
            ...
            const srcWDnero = app?.bridgeStore?.currencies?.find(
              (i) => i.symbol.toUpperCase() === 'WDNERO' && i.chainId === 102,
            )
            app?.bridgeStore?.setSrcCurrency(srcWDnero)
          }
        }, 800)
      }
