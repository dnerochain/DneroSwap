...
interface NonDneroPendingData {
  txid?: string
  lpAddress?: string
  type?: NonDneroFarmStepType
}
export function usePendingTransactions(): {
  hasPendingTransactions: boolean
  pendingNumber: number
  nonDneroFarmPendingList: NonDneroPendingData[]
} {
  const allTransactions = useAllTransactions()
  ...
  const pending = sortedRecentTransactions
    .filter((tx) => !tx.receipt || tx?.nonDneroFarm?.status === FarmTransactionStatus.PENDING)
    .map((tx) => tx.hash)
  const hasPendingTransactions = !!pending.length

  const nonDneroFarmPendingList = sortedRecentTransactions
    .filter((tx) => pending.includes(tx.hash) && !!tx.nonDneroFarm)
    .map((tx) => ({ txid: tx?.hash, lpAddress: tx?.nonDneroFarm?.lpAddress, type: tx?.nonDneroFarm?.type }))

  return {
    hasPendingTransactions,
    nonDneroFarmPendingList,
    pendingNumber: pending.length,
  }
}

export function useNonDneroFarmPendingTransaction(lpAddress: string): NonDneroPendingData[] {
  const { nonDneroFarmPendingList } = usePendingTransactions()
  return useMemo(() => {
    return nonDneroFarmPendingList.filter((tx) => tx?.lpAddress?.toLocaleLowerCase() === lpAddress.toLocaleLowerCase())
  }, [lpAddress, nonDneroFarmPendingList])
}
