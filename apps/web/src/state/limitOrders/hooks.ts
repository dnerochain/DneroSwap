...
import { Currency, CurrencyAmount, Trade, Token, Price, Native, TradeType } from '@dneroswap/sdk'
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency.isToken ? currency.address : currency.isNative ? 'DTOKEN' : '',
        }),
      )
    },
    [dispatch],
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'DTOKEN') return 'DTOKEN'
    if (valid === undefined) return 'DTOKEN'
  }
  return ''
}
