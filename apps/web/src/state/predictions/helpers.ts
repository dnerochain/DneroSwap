...
import { GRAPH_API_PREDICTION_DTOKEN, GRAPH_API_PREDICTION_WDNERO } from 'config/constants/endpoints'
import {
  Bet,
  LedgerData,
  BetPosition,
...
import { transformBetResponseWDNERO, transformUserResponseWDNERO } from './wdneroTransformers'
import { transformBetResponseDTOKEN, transformUserResponseDTOKEN } from './dtokenTransformers'
import { BetResponse, UserResponse } from './responseType'
import { BetResponseDTOKEN } from './dtokenQueries'
import { BetResponseWDNERO } from './wdneroQueries'
...
export const transformBetResponse = (tokenSymbol) =>
  tokenSymbol === 'WDNERO' ? transformBetResponseWDNERO : transformBetResponseDTOKEN

export const transformUserResponse = (tokenSymbol) =>
  tokenSymbol === 'WDNERO' ? transformUserResponseWDNERO : transformUserResponseDTOKEN
...
export const getTotalWon = async (): Promise<{ totalWonDTOKEN: number; totalWonWDNERO: number }> => {
  const [{ market: DTOKENMarket, market: WDNEROMarket }] = await Promise.all([
    request(
      GRAPH_API_PREDICTION_DTOKEN,
      gql`
  ...
  const totalWonDTOKEN = getTotalWonMarket(DTOKENMarket, 'DTOKEN')
  const totalWonWDNERO = getTotalWonMarket(WDNEROMarket, 'WDNERO')

  return { totalWonDTOKEN, totalWonWDNERO }
}
...
export const getBetHistory = async (
  ...
  api: string,
  tokenSymbol: string,
): Promise<Array<BetResponseDTOKEN | BetResponseWDNERO>> => {
...
export const getLedgerData = async (
  ...
  const response = await dneroClient.multicall({
...
export const getPredictionData = async (address: Address): Promise<MarketData> => {
  const [currentEpoch, intervalSeconds, minBetAmount, paused] = await dneroClient.multicall({
    contracts: [
      {
        address,
        abi: predictionsV2ABI,
