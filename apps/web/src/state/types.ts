import { SerializedFarmsState } from '@dneroswap/farms'
import { SerializedPoolWithInfo } from '@dneroswap/pools'
...
export interface SerializedLockedWDneroVault extends Omit<SerializedWDneroVault, 'userData'> {
  totalLockedAmount?: SerializedBigNumber
  userData?: SerializedLockedVaultUser
}

export interface SerializedWDneroVault {
  totalShares?: SerializedBigNumber
  pricePerFullShare?: SerializedBigNumber
  totalWDneroInVault?: SerializedBigNumber
...
export interface PoolsState {
  data: SerializedPool[]
  ifo: IfoState
  wdneroVault: SerializedLockedWDneroVault
  wdneroFlexibleSideVault: SerializedWDneroVault
...
export interface Bet {
  ...
  claimedBlock: number
  claimedHash: string
  claimedDTOKEN: number
  claimedNetDTOKEN: number
  createdAt: number
  updatedAt: number
  user?: PredictionUser
  round?: Round
...
export interface PredictionUser {
  ...
  totalDTOKEN: number
  totalDTOKENBull: number
  totalDTOKENBear: number
  totalBetsClaimed: number
  totalDTOKENClaimed: number
  winRate: number
  averageDTOKEN: number
  netDTOKEN: number
  bets?: Bet[]
}
...
export interface LotteryRound extends LotteryRoundGenerics {
  ...
  priceTicketInWDnero: BigNumber
  discountDivisor: BigNumber
  amountCollectedInWDnero: BigNumber
  wdneroPerBracket: string[]
  countWinnersPerBracket: string[]
  rewardsBreakdown: string[]
}

export interface LotteryResponse extends LotteryRoundGenerics {
  priceTicketInWDnero: SerializedBigNumber
  discountDivisor: SerializedBigNumber
  amountCollectedInWDnero: SerializedBigNumber
  wdneroPerBracket: SerializedBigNumber[]
