...
const fetchWDneroRewardsForTickets = async (
  winningTickets: LotteryTicket[],
): Promise<{ ticketsWithUnclaimedRewards: LotteryTicket[]; wdneroTotal: BigNumber }> => {
  ...
  try {
    ...
    const wdneroRewards = await client.multicall({
      contracts: calls,
      allowFailure: false,
    })

    const wdneroTotal = wdneroRewards.reduce((accum: BigNumber, wdneroReward: bigint) => {
      return accum.plus(new BigNumber(wdneroReward.toString()))
    }, BIG_ZERO)

    const ticketsWithUnclaimedRewards = winningTickets.map((winningTicket, index) => {
      return { ...winningTicket, wdneroReward: wdneroRewards[index].toString() }
    })
    return { ticketsWithUnclaimedRewards, wdneroTotal }
  } catch (error) {
    console.error(error)
    return { ticketsWithUnclaimedRewards: null, wdneroTotal: null }
  }
}
...
export const getWinningTickets = async (
  ...
  if (unclaimedWinningTickets.length > 0) {
    const { ticketsWithUnclaimedRewards, wdneroTotal } = await fetchWDneroRewardsForTickets(unclaimedWinningTickets)
    return { ticketsWithUnclaimedRewards, allWinningTickets, wdneroTotal, roundId }
  }

  if (allWinningTickets.length > 0) {
    return { ticketsWithUnclaimedRewards: null, allWinningTickets, wdneroTotal: null, roundId }
  }
