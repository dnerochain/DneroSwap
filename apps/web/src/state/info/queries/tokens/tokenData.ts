...
interface TokenFields {
  ...
  name: string
  decimals: string
  derivedDTOKEN: string // Price in DTOKEN per token
...
interface FormattedTokenFields
  extends Omit<
    TokenFields,
    'derivedETH' | 'derivedDTOKEN' | 'derivedUSD' | 'tradeVolumeUSD' | 'totalTransactions' | 'totalLiquidity' | 'decimals'
  > {
...
const parseTokenData = (tokens?: TokenFields[]) => {
  ...
  return tokens.reduce((accum: { [address: string]: FormattedTokenFields }, tokenData) => {
    const { derivedDTOKEN, derivedUSD, tradeVolumeUSD, totalTransactions, totalLiquidity, derivedETH, decimals } =
      tokenData
    accum[tokenData.id.toLowerCase()] = {
      ...tokenData,
      derivedDTOKEN: derivedDTOKEN ? 0 : parseFloat(derivedDTOKEN),
