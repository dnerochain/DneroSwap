...
/**
 * Price data for token and dtoken based on block number
 */
...
const fetchTokenPriceData = async (
  ...
  try {
    ...
    if (blocksLength > 0 && chainName === 'DNERO' && !checkIsStableSwap()) {
    ...
    // format token DTOKEN price results
    const tokenPrices: {
      timestamp: string
      derivedDTOKEN: number
      priceUSD: number
    }[] = []

    const mainToken = multiChainQueryMainToken[chainName]

    // Get Token prices in DTOKEN
    Object.keys(prices).forEach((priceKey) => {
      const timestamp = priceKey.split('t')[1]
      // if its DTOKEN price e.g. `b123` split('t')[1] will be undefined and skip DTOKEN price entry
      if (timestamp) {
        tokenPrices.push({
          timestamp,
          derivedDTOKEN: prices[priceKey]?.[`derived${mainToken}`]
    ...
    // Go through DTOKEN USD prices and calculate Token price based on it
    Object.keys(prices).forEach((priceKey) => {
      ...
      if (timestamp) {
        ...
        if (tokenPriceIndex >= 0) {
          const { derivedDTOKEN } = tokenPrices[tokenPriceIndex]
          tokenPrices[tokenPriceIndex].priceUSD =
            parseFloat(prices[priceKey]?.[`${mainToken.toLowerCase()}Price`] ?? 0) * derivedDTOKEN
        }
      }
    })
