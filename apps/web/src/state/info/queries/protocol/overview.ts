...
interface OverviewResponse {
  dneroswapFactories: DneroswapFactory[]
  factories?: DneroswapFactory[]
}
/**
...
const getOverviewData = async (
  ...
  const factoryString = checkIsStableSwap() ? `factories` : `dneroswapFactories`
  try {
...
const formatDneroswapFactoryResponse = (rawDneroswapFactory?: DneroswapFactory[]) => {
  if (rawDneroswapFactory) {
    return rawDneroswapFactory.reduce(
      (acc, cur) => {
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const overviewData = formatDneroswapFactoryResponse(data?.dneroswapFactories)
      const overviewData24 = formatDneroswapFactoryResponse(data24?.dneroswapFactories)
      const overviewData48 = formatDneroswapFactoryResponse(data48?.dneroswapFactories)
...
export const fetchProtocolData = async (chainName: MultiChainName, block24: Block, block48: Block) => {
  ...
  if (data.factories && data.factories.length > 0) data.dneroswapFactories = data.factories
  if (data24.factories && data24.factories.length > 0) data24.dneroswapFactories = data24.factories
  if (data48.factories && data48.factories.length > 0) data48.dneroswapFactories = data48.factories
