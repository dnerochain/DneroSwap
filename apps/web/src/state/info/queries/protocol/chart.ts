...
import { DneroswapDayDatasResponse } from '../types'
 ...
 */
const DNEROSWAP_DAY_DATAS = gql`
  query overviewCharts($startTime: Int!, $skip: Int!) {
    dneroswapDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      date
      dailyVolumeUSD
      totalLiquidityUSD
    }
...
const getOverviewChartData = async (
  ...
  try {
    const { dneroswapDayDatas } = await getMultiChainQueryEndPointWithStableSwap(
      chainName,
    ).request<DneroswapDayDatasResponse>(DNEROSWAP_DAY_DATAS, {
      startTime: multiChainStartTime[chainName],
      skip,
    })
    const data = dneroswapDayDatas.map(mapDayData)
