import { Pair, ERC20Token } from '@dneroswap/sdk'
...
export function useFeeDataWithGasPrice(chainIdOverride?: number): {
  ...
  const { data } = useFeeData({
    chainId,
    enabled: chainId !== ChainId.DNERO && chainId !== ChainId.DNERO_TESTNET,
    watch: true,
  })
...
const DEFAULT_DNERO_GAS_BIGINT = BigInt(GAS_PRICE_GWEI.default)
const DEFAULT_DNERO_TESTNET_GAS_BIGINT = BigInt(GAS_PRICE_GWEI.testnet)
/**
 * Note that this hook will only works well for DTOKEN chain
 */
export function useGasPrice(chainIdOverride?: number): bigint | undefined {
  ...
  const { data: dneroProviderGasPrice = DEFAULT_DNERO_GAS_BIGINT } = useSWR(
    signer && chainId === ChainId.DNERO && userGas === GAS_PRICE_GWEI.rpcDefault && ['dneroProviderGasPrice', signer],
    async () => {
      // @ts-ignore
  ...
  if (chainId === ChainId.DNERO) {
    return userGas === GAS_PRICE_GWEI.rpcDefault ? dneroProviderGasPrice : BigInt(userGas ?? GAS_PRICE_GWEI.default)
  }
  if (chainId === ChainId.DNERO_TESTNET) {
    return DEFAULT_DNERO_TESTNET_GAS_BIGINT
  }
  return undefined
}
...
export function toV2LiquidityToken([tokenA, tokenB]: [ERC20Token, ERC20Token]): ERC20Token {
  return new ERC20Token(tokenA.chainId, Pair.getAddress(tokenA, tokenB), 18, 'WDnero-LP', 'Dneroswap LPs')
}
