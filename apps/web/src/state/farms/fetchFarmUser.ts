...
export const fetchFarmUserAllowances = async (
  ...
  const masterChefAddress = isDneroNetwork ? getMasterChefV2Address(chainId) : getNonDneroVaultAddress(chainId)
...
export const fetchFarmUserStakedBalances = async (
  ...
  const rawStakedBalances = (await publicClient({ chainId }).multicall({
    contracts: farmsToFetch.map((farm) => {
      return {
        abi: isDneroNetwork ? masterChefV2ABI : nonDneroVaultABI,
...
export const fetchFarmUserEarnings = async (
  ...
  const userAddress = isDneroNetwork ? account : await fetchCProxyAddress(account, multiCallChainId)
