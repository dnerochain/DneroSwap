import { createFarmFetcher, SerializedFarm, SerializedFarmsState } from '@dneroswap/farms'
...
const fetchFarmPublicDataPkg = async ({
  ...
  const { farmsWithPrice, poolLength, regularWDneroPerBlock, totalRegularAllocPoint } = await farmFetcher.fetchFarms({
    chainId,
    isTestnet: chain.testnet,
    farms: farmsCanFetch.concat(priceHelperLpsConfig),
  })
  return [farmsWithPrice, poolLength, regularWDneroPerBlock, totalRegularAllocPoint]
}
...
>(
  ...
  async ({ account, pids, proxyAddress, chainId }, { dispatch, getState }) => {
    ...
    const poolLength = state.farms.poolLength ?? (await fetchMasterChefFarmPoolLength(ChainId.DNERO))
    const farmsConfig = await getFarmConfig(chainId)
...
export const farmsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchFarmsPublicDataAsync.fulfilled, (state, action) => {
      const [farmPayload, poolLength, regularWDneroPerBlock, totalRegularAllocPoint] = action.payload
      ...
      state.regularWDneroPerBlock = regularWDneroPerBlock
