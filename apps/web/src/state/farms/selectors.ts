...
import { deserializeFarm, deserializeFarmUserData } from '@dneroswap/farms'
import { State } from '../types'

const selectWDneroFarm = (state: State) => state.farms.data.find((f) => f.pid === 2)
...
export const priceWDneroFromPidSelector = createSelector([selectWDneroFarm], (wdneroDTokenFarm) => {
  const wdneroPriceBusdAsString = wdneroDTokenFarm?.tokenPriceBusd
  return new BigNumber(wdneroPriceBusdAsString)
})
...
export const farmSelector = (chainId: number) =>
  createSelector(
    ...
    (farms) => {
      ...
      const { loadArchivedFarmsData, userDataLoaded, poolLength, regularWDneroPerBlock, totalRegularAllocPoint } = farms
