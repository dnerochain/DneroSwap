...
import { FARM_AUCTION_HOSTING_IN_SECONDS } from '@dneroswap/farms'
...
const fetchFarmsWithAuctions = async (
  ...
  const [auctionDateResponse, auctionBiddersResponse] = await dneroClient.multicall({
    contracts: [
      {
  ...
  if (blocksSinceEnd > 0) {
    const secondsSinceEnd = blocksSinceEnd * DNERO_BLOCK_TIME
