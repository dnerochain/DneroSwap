...
export const transformPotteryPublicData = (publicData: SerializedPotteryPublicData): DeserializedPublicData => {
  const { totalPrize, totalLockWDnero, totalSupply, totalLockedValue, maxTotalDeposit } = publicData

  return {
    ...publicData,
    totalPrize: totalPrize ? new BigNumber(totalPrize) : BIG_ZERO,
    totalLockWDnero: totalLockWDnero ? new BigNumber(totalLockWDnero) : BIG_ZERO,
