...
/**
 * Fetch market data for DneroswapBunnies NFTs by bunny id using the Subgraph
...
export const getNftsByBunnyIdSg = async (
  ...
  try {
    ...
    const res = await request(
      ...
      `,
      {
        collectionAddress: dneroswapBunniesAddress.toLowerCase(),
        where,
        orderDirection,
      },
    )
...
export const getAccountNftsOnChainMarketData = async (
  ...
  try {
    ...
    const askCallsResultsRaw = await publicClient({ chainId: ChainId.DNERO }).multicall({
      contracts: call,
      allowFailure: false,
    })
...
export const getAllDneroswapBunniesLowestPrice = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleDneroswapBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    const rawResponse: Record<string, { currentAskPrice: string }[]> = await request(
      GRAPH_API_NFTMARKET,
      gql`
        query getAllDneroswapBunniesLowestPrice {
          ${singleDneroswapBunnySubQueries}
        }
      `,
    )
    return fromPairs(
    ...
    console.error('Failed to fetch DneroswapBunnies lowest prices', error)
    return {}
  }
}

export const getAllDneroswapBunniesRecentUpdatedAt = async (bunnyIds: string[]): Promise<Record<string, number>> => {
  try {
    const singleDneroswapBunnySubQueries = bunnyIds.map(
      (
        bunnyId,
    ...
    console.error('Failed to fetch DneroswapBunnies latest market updates', error)
    return {}
  }
}
...
export const getMetadataWithFallback = (apiMetadata: ApiResponseCollectionTokens['data'], bunnyId: string) => {
  ...
  return (
    apiMetadata[bunnyId] ?? {
      name: '',
      description: '',
      collection: { name: 'Dneroswap Bunnies' },
      image: {
        original: '',
        thumbnail: '',
      },
...
export const getDneroswapBunniesAttributesField = (bunnyId: string) => {
  ...
  const walletNftsWithMarketData = walletNfts.map((walletNft) => {
    ...
    return (
      marketData ?? {
        ...
        latestTradedPriceInDTOKEN: null,
        tradeVolumeDTOKEN: null,
        totalTrades: null,
        otherId: null,
      }
    )
