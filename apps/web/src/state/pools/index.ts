...
import { getFarmsPrices } from '@dneroswap/farms/farmPrices'
import { fetchPublicIfoData, fetchUserIfoCredit } from '@dneroswap/ifos'
import {
...
const initialState: PoolsState = {
  ...
  wdneroVault: initialPoolVaultState as any,
  ifo: initialIfoState as any,
  wdneroFlexibleSideVault: initialPoolVaultState as any,
}

export const fetchWDneroPoolPublicDataAsync = () => async (dispatch) => {
  const wdneroPrice = parseFloat(await getWDneroPriceFromOracle())

  const stakingTokenPrice = wdneroPrice
  const earningTokenPrice = wdneroPrice
...
export const fetchWDneroPoolUserDataAsync =
      ...
      contracts: [
        {
          ...
          args: [account as Address, getWDneroVaultAddress(chainId)],
        },
        {
          abi: erc20ABI,
          address: dneroTokens.wdnero.address,
...
export const fetchPoolsPublicDataAsync = (chainId: number) => async (dispatch, getState) => {
  try {
    ...
    const dtokenBusdFarms =
      activePriceHelperLpsConfig.length > 0
        ? [...orderBy(farmsV3Data, 'lmPoolLiquidity', 'desc'), ...farmsV2Data].filter(
            (farm) => farm.token.symbol === 'BUSD' && farm.quoteToken.symbol === 'WDTOKEN',
          )
        : []
    const farmsWithPricesOfDifferentTokenPools =
      dtokenBusdFarms.length > 0
        ? getFarmsPrices([...dtokenBusdFarms, ...poolsWithDifferentFarmToken], nativeStableLpMap[chainId], 18)
        : []
    ...
    for (const pool of poolsConfig) {
      ...
      if (earningTokenAddress && !prices[earningTokenAddress] && !isPoolFinished) {
        ...
        if (pool.earningToken.chainId === ChainId.DNERO && pool.earningToken.address === dneroTokens.ace.address) {
          // eslint-disable-next-line no-await-in-loop
...
export const fetchWDneroVaultPublicData = createAsyncThunk<SerializedLockedWDneroVault, ChainId>(
  'wdneroVault/fetchPublicData',
...
export const fetchWDneroFlexibleSideVaultPublicData = createAsyncThunk<SerializedWDneroVault, ChainId>(
  'wdneroFlexibleSideVault/fetchPublicData',
  async (chainId: any): Promise<any> => {
...
export const fetchWDneroVaultFees = createAsyncThunk<SerializedVaultFees, ChainId>(
  'wdneroVault/fetchFees',
  async (chainId: any): Promise<any> => {
...
export const fetchWDneroFlexibleSideVaultFees = createAsyncThunk<SerializedVaultFees, ChainId>(
  'wdneroFlexibleSideVault/fetchFees',
  async (chainId: any): Promise<any> => {
    const vaultFees = await fetchVaultFees({
      chainId,
      provider: getViemClients,
...
export const fetchWDneroVaultUserData = createAsyncThunk<
  SerializedLockedVaultUser,
  { account: Address; chainId: ChainId }
>('wdneroVault/fetchUser', async ({ account, chainId }) => {
...
export const fetchWDneroFlexibleSideVaultUserData = createAsyncThunk<
  SerializedVaultUser,
  { account: Address; chainId: ChainId }
>('wdneroFlexibleSideVault/fetchUser', async ({ account, chainId }) => {
...
export const PoolsSlice = createSlice({
  ...
  reducers: {
    setInitialPoolConfig: (state, action) => {
      ...
      state.wdneroVault = initialPoolVaultState as any
      state.ifo = initialIfoState as any
      state.wdneroFlexibleSideVault = initialPoolVaultState as any
    },
  ...
  extraReducers: (builder) => {
    builder.addCase(resetUserState, (state) => {
      ...
      state.userDataLoaded = false
      state.wdneroVault = { ...state.wdneroVault, userData: initialPoolVaultState.userData as any }
      state.wdneroFlexibleSideVault = { ...state.wdneroFlexibleSideVault, userData: initialPoolVaultState.userData as any }
    })
    builder.addCase(
    ...
    builder.addCase(fetchWDneroVaultPublicData.fulfilled, (state, action: PayloadAction<SerializedLockedWDneroVault>) => {
      state.wdneroVault = { ...state.wdneroVault, ...action.payload }
    })
    builder.addCase(
      fetchWDneroFlexibleSideVaultPublicData.fulfilled,
      (state, action: PayloadAction<SerializedWDneroVault>) => {
        state.wdneroFlexibleSideVault = { ...state.wdneroFlexibleSideVault, ...action.payload }
      },
    )
    // Vault fees
    builder.addCase(fetchWDneroVaultFees.fulfilled, (state, action: PayloadAction<SerializedVaultFees>) => {
      const fees = action.payload
      state.wdneroVault = { ...state.wdneroVault, fees }
    })
    builder.addCase(fetchWDneroFlexibleSideVaultFees.fulfilled, (state, action: PayloadAction<SerializedVaultFees>) => {
      const fees = action.payload
      state.wdneroFlexibleSideVault = { ...state.wdneroFlexibleSideVault, fees }
    })
    // Vault user data
    builder.addCase(fetchWDneroVaultUserData.fulfilled, (state, action: PayloadAction<SerializedLockedVaultUser>) => {
      const userData = action.payload
      state.wdneroVault = { ...state.wdneroVault, userData }
    })
    // IFO
    ...
    builder.addCase(
      fetchWDneroFlexibleSideVaultUserData.fulfilled,
      (state, action: PayloadAction<SerializedVaultUser>) => {
        const userData = action.payload
        state.wdneroFlexibleSideVault = { ...state.wdneroFlexibleSideVault, userData }
      },
    )
    builder.addMatcher(
      isAnyOf(
