...
export const fetchAceTokenPrice = async (tokenAddress: string) => {
  ...
  const blocks = await getBlocksFromTimestamps(timestampsArray, 'desc', 1000, 'DNERO')

  const result = await fetchedTokenDatas(
    v3InfoClients[ChainId.DNERO],
    [tokenAddress.toLowerCase()],
    blocks?.filter((d) => d.number >= SUBGRAPH_START_BLOCK[ChainId.DNERO]),
  )
