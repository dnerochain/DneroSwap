...
import {
  ...
  FarmsV3Response,
  IPendingWDneroByTokenId,
  PositionDetails,
  SerializedFarmsV3Response,
  bWDneroSupportedChainId,
  createFarmFetcherV3,
  supportedChainIdV3,
} from '@dneroswap/farms'
import { priceHelperTokens } from '@dneroswap/farms/constants/common'
import { farmsV3ConfigChainMap } from '@dneroswap/farms/constants/v3'
import { bWDneroFarmBoosterVeWDneroABI } from '@dneroswap/farms/constants/v3/abi/bWDneroFarmBoosterVeWDnero'
import { TvlMap, fetchCommonTokenUSDValue } from '@dneroswap/farms/src/fetchFarmsV3'
...
import { useBWDneroFarmBoosterVeWDneroContract, useMasterchefV3, useV3NFTPositionManagerContract } from 'hooks/useContract'
...
export const useFarmsV3 = ({ mockApr = false }: UseFarmsOptions = {}) => {
  ...
  const { data } = useQuery<FarmsV3Response<FarmV3DataWithPriceTVL>>(
    ...
    async () => {
      ...
      const farmWithPriceAndWDneroAPR = farmV3.data.farmsWithPrice.map((f) => {
        if (!tvls[f.lpAddress]) {
          return f
        }
        ...
        const { activeTvlUSD, activeTvlUSDUpdatedAt, wdneroApr } = farmFetcherV3.getWDneroAprAndTVL(
          f,
          tvl,
          wdneroPrice.toString(),
          farmV3.data.wdneroPerSecond,
        )
      ...
      return {
        ...farmV3.data,
        farmsWithPrice: farmWithPriceAndWDneroAPR,
      }
    },
    {
        ...
        return res.result
          .map((r) =>
            decodeFunctionResult({
              abi: masterchefV3?.abi,
              functionName: zkSyncChains.includes(chainId ?? 0) ? 'pendingWDnero' : 'harvest',
              data: r,
            }),
          )
          .map((r) => {
...
const usePositionsByUserFarms = (
  ...
  const pendingWDneroByTokenIds = useMemo(
    () =>
      (tokenIdResults as bigint[])?.reduce<IPendingWDneroByTokenId>((acc, pendingWDnero, i) => {
        const position = stakedPositions[i]

        return pendingWDnero && position?.tokenId ? { ...acc, [position.tokenId.toString()]: pendingWDnero } : acc
      }, {} as IPendingWDneroByTokenId) ?? {},
  ...
  const farmsWithPositions = useMemo(
    () =>
      farmsV3.map((farm) => {
        ...
        return {
          ...
          pendingWDneroByTokenIds: Object.entries(pendingWDneroByTokenIds).reduce<IPendingWDneroByTokenId>(
            (acc, [tokenId, cake]) => {
    ...
    [farmsV3, pendingWDneroByTokenIds, stakedPositions, unstakedPositions],
  )
...
export function useFarmsV3WithPositionsAndBooster(options: UseFarmsOptions = {}): {
  ...
  return {
    ...
    wdneroPerSecond: data.wdneroPerSecond,
    isLoading,
  }
}
...
const useV3BoostedFarm = (pids?: number[]) => {
  const { chainId } = useActiveChainId()
  const farmBoosterVeWDneroContract = useBWDneroFarmBoosterVeWDneroContract()

  const { data } = useQuery(
    ['v3/boostedFarm', chainId, pids?.join('-')],
    () =>
      getV3FarmBoosterWhiteList({
        farmBoosterContract: farmBoosterVeWDneroContract,
        chainId: chainId ?? -1,
        pids: pids ?? [],
      }),
    {
      enabled: Boolean(chainId && pids && pids.length > 0 && bWDneroSupportedChainId.includes(chainId)),
      retry: 3,
      retryDelay: 3000,
    },
  )
...
export async function getV3FarmBoosterWhiteList({
  ...
  farmBoosterContract: ReturnType<typeof useBWDneroFarmBoosterVeWDneroContract>
  chainId: ChainId
  pids: number[]
