import { Pair, Route, Token, Trade, TradeType, CurrencyAmount } from '@dneroswap/sdk'
import { ChainId } from '@dneroswap/chains'
import { computeTradePriceBreakdown } from 'utils/exchange'

describe('prices', () => {
  const token1 = new Token(ChainId.DNERO, '0x0000000000000000000000000000000000000001', 18, 'Token1')
  const token2 = new Token(ChainId.DNERO, '0x0000000000000000000000000000000000000002', 18, 'Token2')
  const token3 = new Token(ChainId.DNERO, '0x0000000000000000000000000000000000000003', 18, 'Token3')
