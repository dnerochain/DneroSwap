...
import { Token, Percent, CurrencyAmount } from '@dneroswap/sdk'
...
import { ADDRESS_ZERO } from '@dneroswap/v3-sdk'

describe('utils', () => {
  describe('#getDneroScanLink', () => {
    it('correct for tx', () => {
      expect(getBlockExploreLink('abc', 'transaction', ChainId.DNERO)).toEqual('https://explorer.dnerochain.xyz/tx/abc')
    })
    it('correct for token', () => {
      expect(getBlockExploreLink('abc', 'token', ChainId.DNERO)).toEqual('https://explorer.dnerochain.xyz/token/abc')
    })
    it('correct for address', () => {
      expect(getBlockExploreLink('abc', 'address', ChainId.DNERO)).toEqual('https://explorer.dnerochain.xyz/address/abc')
    })
    it('enum', () => {
      expect(getBlockExploreLink('abc', 'address', ChainId.DNERO_TESTNET)).toEqual(
        'https://testnet.explorer.dnerochain.xyz/address/abc',
      )
    })
  })
  ...
  describe('#calculateSlippageAmount', () => {
    it('bounds are correct', () => {
      const tokenAmount = CurrencyAmount.fromRawAmount(new Token(ChainId.DNERO, ADDRESS_ZERO, 0, 'TOKEN'), '100')
