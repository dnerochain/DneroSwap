...
import { Box, Flex, Heading, Skeleton, Balance } from '@dneroswap/uikit'
...
const Hero = () => {
  ...
  const {
    currentRound: { amountCollectedInWDnero, status },
    isTransitioning,
  } = useLottery()

  const wdneroPriceBusd = useWDneroPrice()
  const prizeInBusd = amountCollectedInWDnero.times(wdneroPriceBusd)
