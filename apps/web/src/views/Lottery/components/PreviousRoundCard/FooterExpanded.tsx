...
import { Flex, Skeleton, Heading, Box, Text, Balance } from '@dneroswap/uikit'
...
import { formatNumber, getBalanceNumber } from '@dneroswap/utils/formatBalance'
...
> = ({ lotteryNodeData, lotteryId }) => {
  ...
  if (lotteryNodeData) {
    const { amountCollectedInWDnero } = lotteryNodeData
    prizeInBusd = amountCollectedInWDnero.times(wdneroPriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" WDNERO"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInWDnero)}
            decimals={0}
          />
        )}
      </>
