...
import { BIG_ZERO, BIG_ONE_HUNDRED } from '@dneroswap/utils/bigNumber'
...
const BuyTicketsModal: React.FC<React.PropsWithChildren<BuyTicketsModalProps>> = ({ onDismiss }) => {
  ...
  const stringifiedUserWDnero = userWDnero.toJSON()
  const memoisedUserWDnero = useMemo(() => new BigNumber(stringifiedUserWDnero), [stringifiedUserWDnero])
  ...
  const validateInput = useCallback(
    ...
    [limitNumberByMaxTicketsPerBuy, getTicketCostAfterDiscount, maxNumberTicketsPerBuyOrClaim, userWDnero],
  )

  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserWDnero.div(priceTicketInWDnero)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase = limitedMaxPurchase

      // If the users' max WDNERO balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInWDnero.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInWDnero, discountDivisor, getTicketCostAfterDiscount])
  ...
  const getErrorMessage = () => {
    if (userNotEnoughWDnero) return t('Insufficient WDNERO balance')
  ...
  const disableBuying = useMemo(
    ...
    [isConfirming, userNotEnoughWDnero, ticketsToBuy, getTicketsForPurchase],
  )
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
      ...
      <BalanceInput
        isWarning={account && (userNotEnoughWDnero || maxTicketPurchaseExceeded)}
        placeholder="0"
        value={ticketsToBuy}
        onUserInput={handleInputChange}
        currencyValue={
          wdneroPriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInWDnero.times(new BigNumber(ticketsToBuy))) : '0.00'} WDNERO`
        }
      />
      <Flex alignItems="center" justifyContent="flex-end" mt="4px" mb="12px">
        <Flex justifyContent="flex-end" flexDirection="column">
          {account && (userNotEnoughWDnero || maxTicketPurchaseExceeded) && (
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInWDnero && getFullDisplayBalance(priceTicketInWDnero.times(ticketsToBuy || 0))} WDNERO
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
