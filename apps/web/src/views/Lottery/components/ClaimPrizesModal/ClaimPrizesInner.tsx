...
import { AutoRenewIcon, Button, Flex, PresentWonIcon, Text, useToast, Balance } from '@dneroswap/uikit'
...
const ClaimInnerContainer: React.FC<React.PropsWithChildren<ClaimInnerProps>> = ({ onSuccess, roundsToClaim }) => {
  ...
  const wdneroReward = activeClaimData.wdneroTotal
  const dollarReward = wdneroReward.times(wdneroPriceBusd)
  const rewardAsBalance = getBalanceAmount(wdneroReward).toNumber()
    ...
    if (receipt?.status) {
      toastSuccess(
        ...
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('Your WDNERO prizes for round %lotteryId% have been sent to your wallet', { lotteryId })}
