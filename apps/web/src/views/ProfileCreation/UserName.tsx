...
const UserName: React.FC<React.PropsWithChildren> = () => {
  ...
  const hasMinimumWDneroRequired = fetchStatus === FetchStatus.Fetched && wdneroBalance >= REGISTER_COST
    ...
    <>
      ...
      {!hasMinimumWDneroRequired && (
        <Text color="failure" mt="16px">
          {t('A minimum of %num% WDNERO is required', { num: formatUnits(REGISTER_COST, 18) })}
        </Text>
      )}
    </>
  )
