...
import { Flex, Text, InfoIcon, Message, MessageText } from '@dneroswap/uikit'
...
import AddWDneroButton from 'views/Pools/components/LockedPool/Buttons/AddWDneroButton'
...
import useRewardInWDnero from 'views/TradingReward/hooks/useRewardInWDnero'
...
const QualifiedPreview: React.FC<React.PropsWithChildren<QualifiedPreviewProps>> = ({
  ...
  const rewardInWDnero = useRewardInWDnero({
    timeRemaining,
    totalEstimateRewardUSD: currentUserCampaignInfo?.totalEstimateRewardUSD ?? 0,
  ...
  const maxRewardCapWDneroPrice = useMemo(
    () => new BigNumber(maxRewardCap).div(wdneroPriceBusd).toNumber(),
    [wdneroPriceBusd, maxRewardCap],
  )
  ...
  return (
    <>
      <GreyCard>
        ...
        <Text fontSize="14px" color="textSubtle">{`~${formatNumber(rewardInWDnero)} WDNERO`}</Text>
        ...
        {additionalAmount >= 0.01 && (
          <Message variant="warning" mt="10px">
            <MessageText>
              ...
              <Text as="span" bold>
                {t('Lock more WDNERO to keep earning.')}
              </Text>
            </MessageText>
          </Message>
        )}
      ...
      {additionalAmount >= 0.01 && (
        <GreyCard mt="24px">
          ...
          <Text color="failure" fontSize="14px">{`~${formatNumber(maxRewardCapWDneroPrice)} WDNERO`}</Text>
          <Text width="100%" lineHeight="120%">
            <Text color="textSubtle" fontSize="14px" lineHeight="120%" as="span">
              {t('Equals to your %amount%% of locked WDNERO divided by', { amount: maxRewardCapInfoAmount })}
            </Text>
            <Text color="textSubtle" fontSize="14px" lineHeight="120%" as="span" ml="4px">
              {t('of the amount of your locked WDNERO. Lock more WDNERO to raise this limit')}
            </Text>
          </Text>
          {additionalAmount >= 0.01 && (
            <AddWDneroButton
