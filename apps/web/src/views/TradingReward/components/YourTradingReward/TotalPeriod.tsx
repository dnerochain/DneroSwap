...
import { Box, Flex, Card, Text, Message, MessageText, TooltipText, Button, useTooltip } from '@dneroswap/uikit'
...
const TotalPeriod: React.FC<React.PropsWithChildren<TotalPeriodProps>> = ({
    ...
    return (
      totalAvailableClaimData
        ...
        .filter((campaign) => {
          ...
          if (type === RewardType.WDNERO_STAKERS && claimedRebate && isValid) {
            return campaign
          }
  ...
  const getUSDValue = useCallback(
    (userCampaignInfoDetail: UserCampaignInfoDetail) => {
      ...
      if (currentReward) {
        const rewardWDneroUSDPriceAsBg = getBalanceAmount(
          new BigNumber(currentReward.rewardPrice),
          currentReward.rewardTokenDecimal,
        )
        const rewardWDneroAmount = getBalanceAmount(
          new BigNumber(userCampaignInfoDetail?.canClaim),
          currentReward.rewardTokenDecimal,
        )
        return rewardWDneroAmount.times(rewardWDneroUSDPriceAsBg).toNumber() || 0
      }
      return 0
    },
    [rewardInfo],
      ...
      <Card style={{ width: '100%' }}>
        <Box padding={['24px']}>
            ...
            <Flex flexDirection={['column', 'column', 'column', 'row']}>
              <Box>
                ...
                <Text fontSize={['14px']} color="textSubtle">
                  {minAmountDisplay({ amount: totalUnclaimInWDnero, prefix: '~', unit: ' WDNERO' })}
                </Text>
              </Box>
              <Button
              ...
              <Message variant="warning" mt="16px">
                <MessageText>
                  <Text m="0 4px" as="span">
                    {t('Please switch your network to DneroChain to claim your trading rewards.')}
                  </Text>
                </MessageText>
              </Message>
            )}
          ...
          <GreyCard mt="24px">
            ...
            {type === RewardType.WDNERO_STAKERS && (
              <Box mt="24px">
                <Text color="textSubtle" textTransform="uppercase" fontSize="12px" bold>
