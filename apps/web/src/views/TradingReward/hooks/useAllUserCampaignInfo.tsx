...
const useAllUserCampaignInfo = ({ campaignIds, type }: UseAllUserCampaignInfoProps): AllUserCampaignInfo => {
  ...
  const tradingRewardAddress =
    type === RewardType.WDNERO_STAKERS
      ? getTradingRewardAddress(ChainId.DNERO)
      : getTradingRewardTopTradesAddress(ChainId.DNERO)
      ...
      try {
        const allUserCampaignInfo = await Promise.all(
          campaignIds.map(async (campaignId: string) => {
            ...
            const userClaimedIncentives = await dneroClient.readContract({
              abi: tradingRewardABI,
            ...
            const canClaimResult = await dneroClient.multicall({
