...
const MoboxUserPrizeGrid: React.FC<React.PropsWithChildren<{ userTradingInformation?: UserTradingInformation }>> = ({
  ...
  const { userRewardGroup, userWDneroRewards, userMoboxRewards, userPointReward, canClaimMysteryBox, canClaimNFT } =
    userTradingInformation
  const { wdneroReward, moboxReward, dollarValueOfTokensReward } = useMoboxCompetitionRewards({
    userWDneroRewards,
    userMoboxRewards,
  })
