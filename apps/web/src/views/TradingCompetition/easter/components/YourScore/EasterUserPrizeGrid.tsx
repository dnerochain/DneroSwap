...
import { useCompetitionWDneroRewards, getEasterRewardGroupAchievements } from '../../../helpers'
...
const EasterUserPrizeGrid: React.FC<React.PropsWithChildren<{ userTradingInformation? }>> = ({
  ...
  const { userRewardGroup, userWDneroRewards, userPointReward, canClaimNFT } = userTradingInformation
  const { wdneroReward, dollarValueOfWDneroReward } = useCompetitionWDneroRewards(userWDneroRewards)
        ...
        <tr>
          <BoldTd>
            <Flex flexDirection="column">
              <Text bold>{wdneroReward.toFixed(2)}</Text>
              {dollarValueOfWDneroReward ? (
                <Text fontSize="12px" color="textSubtle">
                  ~{dollarValueOfWDneroReward} USD
                </Text>
              ) : (
