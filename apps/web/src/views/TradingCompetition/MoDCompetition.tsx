...
import { Flex, Box, useMatchBreakpoints, PageSection } from '@dneroswap/uikit'
...
import MoDWDnerorBunny from './pngs/MoD-wdneror.png'
...
const MoDCompetition = () => {
  ...
  const { hasUserClaimed, isUserActive, userWDneroRewards, userDarRewards, userPointReward, canClaimNFT } =
  ...
  const userCanClaimPrizes =
    ...
    isUserActive &&
    !hasUserClaimed &&
    (userWDneroRewards !== '0' || userDarRewards !== '0' || userPointReward !== '0' || canClaimNFT)
  ...
  useEffect(() => {
    ...
    const fetchUserContract = async () => {
      try {
        ...
        const [user, userClaimed] = await dneroClient.multicall({
          contracts: [
            {
        ...
        const userObject: UserTradingInformation = {
          ...
          userWDneroRewards: user[4].toString(),
