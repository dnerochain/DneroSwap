import { Box, BunnyPlaceholderIcon, Flex, Text } from '@dneroswap/uikit'
import TradingView, { useTradingViewEvent } from 'components/TradingView'
import { useTranslation } from '@dneroswap/localization'
import { useDebounce } from '@dneroswap/hooks'
...
const dtokenToWDTOKENSymbol = (sym: string) => (sym === 'DTOKEN' ? 'WDTOKEN' : sym)

const ID = 'TV_SWAP_CHART'
const SYMBOL_PREFIX = 'DNEROSWAP:'
...
const TradingViewChart = ({ outputSymbol, inputSymbol, isDark, onTwChartSymbol }: TradingViewChartProps) => {
  ...
  const symbol = useMemo(() => {
    ...
    const input = dtokenToWDTOKENSymbol(inputSymbol)
    const output = dtokenToWDTOKENSymbol(outputSymbol)
    return `${input}${output}`
