import { Flex, Box, Text, useTooltip } from '@dneroswap/uikit'
import { useTranslation } from '@dneroswap/localization'
import Image from 'next/image'
import { useRouter } from 'next/router'
import { ComputedFarmConfigV3 } from '@dneroswap/farms'
...
const TradingRewardIcon = ({ pairs }: TradingRewardIconProps) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Flex flexDirection="column" width={300}>
      <Text bold>{t('This token contains trading pair(s) eligible for earning WDNERO by trading:')}</Text>
