...
import { Token, TradeType, Currency } from '@dneroswap/sdk'
...
import { FeeOptions } from '@dneroswap/v3-sdk'
...
const addresses = {
  // MetaSwapWrapper
  5647: '0xC0ffeE00c3F5A11369EeB57693C56Fd939dc6DBb',
}
const permitAddresses = {
  5647: '0x31c2F6fcFf4F8759b3Bd5Bf0e1084A055615c768',
}

const originators = {
  5647: ['0xaAB27a41646A4b7e660f2BFc6e22a41550665fef'],
}
...
function useWallchainSDK() {
  ...
  const { data: wallchainSDK } = useQuery(
    ...
    {
      enabled: Boolean(chainId === ChainId.DNERO && walletClient && WALLCHAIN_ENABLED),
...
export function useWallchainApi(
  ...
  useEffect(() => {
    ...
    if (trade.routes.length === 0 || trade.inputAmount.currency.chainId !== ChainId.DNERO) return
