...
import { VeWDneroBenefitCard } from 'views/WDneroStaking/components/SyrupPool/VeWDneroCard'
...
> = memo(({ vaultKey, account, initialActivity }) => {
  ...
  const { totalWDneroInVault } = useVaultPoolByKey(vaultKey)
  const isUserDelegated = useIsUserDelegated()

  const { stakingToken, totalStaked } = pool

  const totalStakedBalance = useMemo(() => {
    return getBalanceNumber(totalWDneroInVault, stakingToken.decimals)
  }, [stakingToken.decimals, totalWDneroInVault])
      ...
      {!account || isUserDelegated ? (
        <MigrateCell>
          {isMobile ? (
            ...
            <VeWDneroBenefitCard isTableView />
          )}
        </MigrateCell>
      ) : null}
      {account && !isUserDelegated && (
