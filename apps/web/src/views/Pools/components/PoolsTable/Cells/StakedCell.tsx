import { Balance, Box, Flex, HelpIcon, Skeleton, Text, useMatchBreakpoints, useTooltip } from '@dneroswap/uikit'
...
const StakedCell: React.FC<React.PropsWithChildren<StakedCellProps>> = ({ pool, account }) => {
  ...
  const {
    userShares,
    balance: { wdneroAsBigNumber, wdneroAsNumberBalance },
    isLoading,
  ...
  const isLocked = pool.vaultKey === VaultKey.WDneroVault && vaultData.userData?.locked
                ...
                <Flex>
                  <Balance
                    ...
                    value={
                      hasStaked
                        ? pool.vaultKey
                          ? Number.isNaN(wdneroAsNumberBalance)
                            ? 0
