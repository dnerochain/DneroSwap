...
import { VaultPosition, getVaultPosition, isLocked } from 'utils/wdneroPool'
...
const AutoAprCell: React.FC<React.PropsWithChildren<AprCellProps>> = ({ pool }) => {
  ...
  const [onPresentLockedApyModal] = useModal(
    ...
    true,
    true,
    pool.vaultKey === VaultKey.WDneroVault ? 'LockedVaultRoiCalculatorModal' : 'FlexibleSideVaultRoiCalculatorModal',
  )

  const tooltipContent = <LockedAprTooltipContent />
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })

  if (pool.vaultKey === VaultKey.WDneroVault && vaultPosition === VaultPosition.None) {
    return (
      <>
