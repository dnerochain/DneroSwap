...
import { DeserializedLockedWDneroVault, DeserializedLockedVaultUser, VaultKey } from 'state/types'
import { VaultPosition, getVaultPosition } from 'utils/wdneroPool'
import {
  LearnMoreLink,
  VeWDneroBunny,
  VeWDneroButton,
  VeWDneroCardTableView,
  VeWDneroMigrateCard,
  VeWDneroUpdateCard,
  VeWDneroUpdateCardTableView,
} from 'views/WDneroStaking/components/SyrupPool'
...
const ActionPanel: React.FC<React.PropsWithChildren<ActionPanelProps>> = ({ account, pool, expanded }) => {
  ...
  const vaultData = useVaultPoolByKey(vaultKey as Pool.VaultKey) as DeserializedLockedWDneroVault
  ...
  const poolStakingTokenBalance = vaultKey
    ? wdneroAsBigNumber.plus(stakingTokenBalance)
    : stakedBalance.plus(stakingTokenBalance)

  const originalLockedAmount = getBalanceNumber(vaultData.userData?.lockedAmount)

  const isWDneroPool = useMemo(
    () => pool.vaultKey === VaultKey.WDneroVault || pool.vaultKey === VaultKey.WDneroFlexibleSideVault,
    [pool.vaultKey],
  )
  ...
  return (
    <StyledActionPanel expanded={expanded}>
      <InfoSection>
        {isMobile && vaultKey === VaultKey.WDneroVault && isLocked && (
          <Box mb="16px">
        ...
        <Flex flexDirection="column" mb="8px">
          <>
            {!isMobile && vaultKey === VaultKey.WDneroVault && !account ? (
              <VeWDneroBunny />
            ) : (
      ...
      <ActionContainer>
        <Box width="100%">
          {pool.vaultKey === VaultKey.WDneroVault && !isUserDelegated && (
        ...
        {!isUserDelegated && isWDneroPool && account && vaultPosition !== VaultPosition.None && (
          <Flex width="100%">
            <Message
              variant="warning"
              ...
              action={
                ...
                >
                  ...
                  {vaultPosition === VaultPosition.Flexible && <VeWDneroUpdateCard isFlexibleStake isTableView />}
                  {vaultPosition >= VaultPosition.LockedEnd && <VeWDneroUpdateCardTableView />}
                  {vaultPosition >= VaultPosition.LockedEnd && <WithdrawAllButton />}
                  <VeWDneroButton style={{ flexBasis: '50%' }} type="get" />
                </Flex>
              }
