...
import { VeWDneroButton } from 'views/WDneroStaking/components/SyrupPool/VeWDneroButton'
...
import VaultStakeModal from '../../WDneroVaultCard/VaultStakeModal'
...
const Staked: React.FunctionComponent<React.PropsWithChildren<StackedActionProps>> = ({ pool }) => {
  ...
  const vaultData = useVaultPoolByKey(vaultKey) as DeserializedLockedWDneroVault

  const userShares = vaultData.userData?.userShares ?? new BigNumber(0)
  const wdneroAsBigNumber = vaultData.userData?.balance?.wdneroAsBigNumber ?? new BigNumber(0)
  const wdneroAsNumberBalance = vaultData.userData?.balance?.wdneroAsNumberBalance ?? 0

  const { lockEndDate, remainingTime, burnStartTime } = useUserDataInVaultPresenter({
    lockStartTime: vaultKey === VaultKey.WDneroVault ? vaultData.userData?.lockStartTime ?? '0' : '0',
    lockEndTime: vaultKey === VaultKey.WDneroVault ? vaultData.userData?.lockEndTime ?? '0' : '0',
    burnStartTime: vaultKey === VaultKey.WDneroVault ? vaultData.userData?.burnStartTime ?? '0' : '0',
  })

  const hasSharesStaked = userShares.gt(0)
  const isVaultWithShares = vaultKey && hasSharesStaked
  const stakedAutoDollarValue = getBalanceNumber(wdneroAsBigNumber.multipliedBy(stakingTokenPrice), stakingToken.decimals)

  const needsApproval = vaultKey ? !isVaultApproved : !allowance.gt(0) && !isDTokenPool
  ...
  const [onPresentVaultUnstake] = useModal(<VaultStakeModal stakingMax={wdneroAsBigNumber} pool={pool} isRemovingStake />)
  ...
  const isLocked = vaultKey === VaultKey.WDneroVault && vaultData.userData?.locked
  const vaultPosition = getVaultPosition(vaultData.userData)
  if (
    !account ||
    (account &&
      vaultPosition === VaultPosition.None &&
      (vaultKey === VaultKey.WDneroVault || vaultKey === VaultKey.WDneroFlexibleSideVault)) ||
    isUserDelegated
  ) {
    if (isMobile) {
      return vaultKey === VaultKey.WDneroVault || vaultKey === VaultKey.WDneroFlexibleSideVault ? (
                ...
                <Box position="relative">
                  <Flex>
                    <Balance
                      ...
                      fontSize="20px"
                      decimals={5}
                      value={vaultKey ? wdneroAsNumberBalance : stakedTokenBalance}
                    />
                    {isLocked ? (
                      <>
                        {' '}
              ...
              {/* {vaultPosition === VaultPosition.Locked && (
                <Box mt="16px">
                  <AddWDneroButton
                    ...
                    lockStartTime={(vaultData as DeserializedLockedWDneroVault).userData.lockStartTime}
                    currentLockedAmount={wdneroAsBigNumber}
                    stakingToken={stakingToken}
                    stakingTokenPrice={stakingTokenPrice}
                    currentBalance={stakingTokenBalance}
                    stakingTokenBalance={stakingTokenBalance}
      ...
      <ActionContent>
        {vaultKey ? (
          <VaultStakeButtonGroup
            ...
            onLockedClick={vaultKey === VaultKey.WDneroVault ? openPresentLockedStakeModal : () => {}}
          />
        ) : (
          <Button
            width="100%"
