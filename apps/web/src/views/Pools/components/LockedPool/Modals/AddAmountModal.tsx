...
import { Modal, Box, MessageText, Message, Checkbox, Flex, Text } from '@dneroswap/uikit'
...
import { getBalanceNumber, getDecimalAmount, getBalanceAmount } from '@dneroswap/utils/formatBalance'
      ...
      {!checkedState && (
        <Message variant="warning" mb="16px">
          <MessageText maxWidth="320px">
            {t(
              'Adding more WDNERO will renew your lock, setting it to remaining duration. Due to shorter lock period, benefits decrease. To keep similar benefits, extend your lock.',
            )}
          </MessageText>
        </Message>
      )}
...
const AddAmountModal: React.FC<React.PropsWithChildren<AddAmountModalProps>> = ({
  ...
  const { allowance } = useCheckVaultApprovalStatus(VaultKey.WDneroVault)
  ...
  return (
    <RoiCalculatorModalProvider lockedAmount={lockedAmount}>
      <Modal title={t('Add WDNERO')} onDismiss={onDismiss} headerBackground={theme.colors.gradientCardHeader}>
        <Box mb="16px">
          <BalanceField
