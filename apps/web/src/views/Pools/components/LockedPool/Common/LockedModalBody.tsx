...
import { Button, AutoRenewIcon, Box, Flex, Message, MessageText, Text } from '@dneroswap/uikit'
import _noop from 'lodash/noop'
import { useTranslation } from '@dneroswap/localization'
import isUndefinedOrNull from '@dneroswap/utils/isUndefinedOrNull'
import { MAX_LOCK_DURATION } from '@dneroswap/pools'
...
const LockedModalBody: React.FC<React.PropsWithChildren<LockedModalBodyPropsType>> = ({
  ...
  const { allowance, setLastUpdated } = useCheckVaultApprovalStatus(VaultKey.WDneroVault)
  const { handleApprove, pendingTx: approvePendingTx } = useVaultApprove(VaultKey.WDneroVault, setLastUpdated)
    ...
    <>
      ...
      {!needsApprove && cakeNeeded ? (
        hasEnoughBalanceToExtend ? (
          <Text fontSize="12px" mt="24px">
            {t('0.0001 WDNERO will be spent to extend')}
          </Text>
        ) : (
          <Message variant="warning" mt="24px">
            <MessageText maxWidth="200px">{t('0.0001 WDNERO required for enabling extension')}</MessageText>
          </Message>
        )
      ) : null}
      ...
      <Flex mt="24px" flexDirection="column">
        {needsApprove ? (
          ...
          <ExtendEnable
            hasEnoughWDnero={hasEnoughBalanceToExtend}
