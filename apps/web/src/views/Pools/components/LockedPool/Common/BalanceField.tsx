import { BalanceInput, Button, Flex, Image, Slider, Text } from '@dneroswap/uikit'
...
import { useUserEnoughWDneroValidator } from '../hooks/useUserEnoughWDneroValidator'
...
const BalanceField: React.FC<React.PropsWithChildren<PropsType>> = ({
  ...
  const { userNotEnoughWDnero, notEnoughErrorMessage } = useUserEnoughWDneroValidator(lockedAmount, stakingTokenBalance)
  ...
  return (
    <>
      ...
      <BalanceInput
        isWarning={userNotEnoughWDnero || needApprove}
        value={lockedAmount}
        onUserInput={handleStakeInputChange}
        currencyValue={`~${usdValueStaked || 0} USD`}
        decimals={stakingDecimals}
      />
      {needApprove && !userNotEnoughWDnero ? (
