...
import { Box, Text, Flex, MessageText, Message } from '@dneroswap/uikit'
...
import formatIWDnero from '../../utils/formatIWDnero'
...
const Overview: React.FC<React.PropsWithChildren<OverviewPropsType>> = ({
  ...
  const formattediWDnero = useMemo(() => {
    return formatIWDnero({ lockedAmount, duration, ceiling }) || 0
  }, [lockedAmount, duration, ceiling])

  const newFormattediWDnero = useMemo(() => {
    ...
    return formatIWDnero({ lockedAmount: amount, duration: lockDuration, ceiling }) || 0
  ...
  return (
    <>
      <Box>
        ...
        <LightGreyCard>
          <BalanceRow title={t('WDnero to be locked')} value={lockedAmount} newValue={newLockedAmount} decimals={2} />
          <BalanceRow title="iWDnero" decimals={2} value={formattediWDnero} newValue={newFormattediWDnero} />
          <BalanceRow
            title="apr"
            unit="%"
      ...
      {showLockWarning && (
        <Box mt="16px" maxWidth="370px">
          <Message variant="warning">
            <MessageText>
              {t('You will be able to withdraw the staked WDNERO and profit only when the staking position is unlocked')}
            </MessageText>
          </Message>
        </Box>
      )}
