...
import { BalanceWithLoading, Box, ButtonVariant, Flex, HelpIcon, Text, useTooltip } from '@dneroswap/uikit'
...
import { VeWDneroMigrateCard } from 'views/WDneroStaking/components/SyrupPool'
...
const LockedStaking: React.FC<React.PropsWithChildren<LockedStakingProps>> = ({ buttonVariant, pool, userData }) => {
  ...
  const currentLockedAmountAsBigNumber = useMemo(() => {
    return userData?.balance?.wdneroAsBigNumber
  }, [userData?.balance?.wdneroAsBigNumber])
    ...
    <Box>
      ...
      <Box mb="16px">
        {position < VaultPosition.LockedEnd ? (
          <VeWDneroMigrateCard lockEndTime={userData?.lockEndTime} />
        ) : (
          <LockedActions
            userShares={userData?.userShares}
            locked={userData?.locked}
