...
import { getRoi } from '@dneroswap/utils/compoundApyHelpers'
...
export const VaultRoiCalculatorModal = ({
  ...
  const [wdneroVaultView, setWDneroVaultView] = useState(initialView || 0)
  ...
  const apy = useMemo(() => {
    return wdneroVaultView === 0 ? flexibleApy : getLockedApy(duration)
  }, [wdneroVaultView, getLockedApy, flexibleApy, duration])

  const [isMaxSelected, setIsMaxSelected] = useState(false)

  return (
    <RoiCalculatorModal
      isLocked={wdneroVaultView === 1}
      account={account}
      ...
      stakingTokenBalance={
        pool.userData?.stakingTokenBalance ? wdneroAsBigNumber.plus(pool.userData?.stakingTokenBalance) : wdneroAsBigNumber
      }
      ...
      header={
        pool.vaultKey === VaultKey.WDneroVault ? (
          <ButtonMenu
            ...
            scale="sm"
            variant="subtle"
            activeIndex={wdneroVaultView}
            onItemClick={setWDneroVaultView}
          >
            {buttonMenuItems}
          </ButtonMenu>
        ) : (
