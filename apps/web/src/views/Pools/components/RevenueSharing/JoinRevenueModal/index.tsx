...
import { Modal, Text, Card, Flex, Box, LinkExternal, useMatchBreakpoints } from '@dneroswap/uikit'
        ...
        <Flex position="relative" zIndex="1" bottom="-5px" m={['auto', 'auto', 'auto', '0 0 0 auto']}>
          <TooltipContainer>
            <Text fontSize={['14px', '14px', '14px', '16px']} lineHeight="110%">
              {
                'Update your WDNERO staking position to join the revenue sharing program for weekly revenue sharing distributions! '
              }
            </Text>
          </TooltipContainer>
          <Image
            alt="lockWDneroTooltip"
        ...
        {!vaultPool?.userData?.isLoading && (
          <Card>
            <Box padding="16px 16px 0 16px">
              <LockedStaking buttonVariant="secondary" pool={wdneroPool} userData={vaultPool?.userData} />
            </Box>
          </Card>
        )}
