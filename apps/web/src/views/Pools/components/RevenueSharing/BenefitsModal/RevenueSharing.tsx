...
import { Balance, Box, Card, Flex, LinkExternal, Message, MessageText, Text, WarningIcon } from '@dneroswap/uikit'
...
const RevenueSharing: React.FunctionComponent<React.PropsWithChildren<RevenueSharingProps>> = ({ onDismiss }) => {
  ...
  const availableWDnero = useMemo(() => getBalanceAmount(new BigNumber(availableClaim)).toNumber(), [availableClaim])
  const availableWDneroUsdValue = useMemo(
    () => new BigNumber(availableWDnero).times(wdneroPriceBusd).toNumber(),
    [availableWDnero, wdneroPriceBusd],
  )
      ...
      <Box padding={16}>
        ...
        <Box mt="8px">
          ...
          <Flex mt="8px" flexDirection="row" alignItems="center">
            ...
            />
            <Box>
              {availableWDnero > 0 && availableWDnero <= 0.01 ? (
                <Text bold textAlign="right">{`< 0.01 WDNERO`}</Text>
              ) : (
                <Balance unit=" WDNERO" textAlign="right" bold value={availableWDnero} decimals={2} />
              )}
              <Balance
                ml="4px"
                color="textSubtle"
