...
export const useUserVote = (gauge?: Gauge, useProxyPool: boolean = true) => {
  ...
  const { data } = useQuery(
    ['/vewdnero/userVoteSlopes', contract.address, gauge?.hash, account],
    async (): Promise<VotedSlope> => {
      const hasProxy = useProxyPool && userInfo?.wdneroPoolProxy && !isAddressEqual(userInfo?.wdneroPoolProxy, zeroAddress)
      const calls = [
        {
          ...contract,
      ...
      const callsWithProxy = [
        {
          ...contract,
          functionName: 'voteUserSlopes',
          args: [userInfo?.wdneroPoolProxy as Address, gauge?.hash as Hex],
        },
        {
          ...contract,
          functionName: 'lastUserVote',
