import { gaugesVotingABI } from '@dneroswap/gauges'
...
export const useUserVoteSlopes = () => {
  ...
  const { data, refetch } = useQuery(
    ['/vewdnero/user-vote-slopes', gaugesVotingContract.address, account, gauges?.length, userInfo?.wdneroPoolProxy],
    async (): Promise<VoteSlope[]> => {
      if (!gauges || gauges.length === 0 || !account) return []

      const hasProxy = userInfo?.wdneroPoolProxy && !isAddressEqual(userInfo?.wdneroPoolProxy, zeroAddress)
      ...
      if (hasProxy) {
        gauges.forEach((gauge) => {
          contracts.push({
            ...gaugesVotingContract,
            functionName: 'voteUserSlopes',
            args: [userInfo.wdneroPoolProxy, gauge.hash as Hex],
          } as const)
        })
      }
