...
import { useGaugesVotingCount } from 'views/WDneroStaking/hooks/useGaugesVotingCount'
...
export const VoteTable = () => {
  ...
  const showNoWDneroLockedWarning = useMemo(() => {
    return !wdneroLocked && rowsWithLock?.length
  }, [wdneroLocked, rowsWithLock])

  const showOnTallyWarning = useMemo(() => {
    return onTally && rowsWithLock?.length && wdneroLocked
  }, [wdneroLocked, onTally, rowsWithLock?.length])
  ...
  return (
    <>
      ...
      <ResponsiveCard>
        ...
        {rowsWithLock?.length && epochPower <= 0n && wdneroLockedAmount > 0n ? (
            ...
            <Message variant="warning" showIcon>
              <AutoColumn gap="8px">
                ...
                <FlexGap alignItems="center" gap="0.2em">
                  {t('To cast your vote, ')}
                  <Link href="/wdnero-staking" color="text">
        ...
        {showNoWDneroLockedWarning ? (
          <Box width={['100%', '100%', '100%', '50%']} px={['16px', 'auto']} my={['24px', '24px', '36px']} mx="auto">
            <Message variant="warning" showIcon>
              <AutoColumn gap="8px">
                <Text>{t('You have no locked WDNERO.')}</Text>
