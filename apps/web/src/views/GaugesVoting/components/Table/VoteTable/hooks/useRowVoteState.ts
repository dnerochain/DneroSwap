...
import formatLocalisedCompactNumber, { getBalanceNumber } from '@dneroswap/utils/formatBalance'
...
import { useProxyVeWDneroBalance } from 'views/WDneroStaking/hooks/useProxyVeWDneroBalance'
...
export const useRowVoteState = ({ data, vote, onChange }: RowProps) => {
  ...
  const { balance: proxyVeWDneroBalance } = useProxyVeWDneroBalance()
  ...
  const previewVoteWeightAmount = useMemo(() => {
    ...
    let balance = veWDneroBalance
    if (userVote?.ignoredSide === 'proxy') {
      balance = veWDneroBalance.minus(proxyVeWDneroBalance)
    }
    if (userVote?.ignoredSide === 'native') {
      balance = proxyVeWDneroBalance
    }
    const amount = getBalanceNumber(balance.times(p).div(10000))
    return amount
  }, [proxyVeWDneroBalance, userVote?.ignoredSide, veWDneroBalance, voteValue])
