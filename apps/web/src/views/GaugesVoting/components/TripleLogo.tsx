...
import {
  ...
  GaugeV2Config,
  GaugeV3Config,
} from '@dneroswap/gauges'
import { Native, Token, WNATIVE } from '@dneroswap/sdk'
import { ChainLogo, CurrencyLogo } from '@dneroswap/widgets-internal'
...
const SingleLogo = ({ size = 32 }: { size?: number }) => {
  return <img src="/images/wdnero-staking/token-vewdnero.png" alt="ve-wdnero" width={size} height={size} />
}
...
const TokensLogo: React.FC<{ gaugeConfig?: GaugeConfig; size?: number }> = ({ gaugeConfig, size }) => {
  ...
  if (gaugeConfig?.type === GaugeType.VeWDneroPool) {
