import { DNERO_BLOCK_TIME, DEFAULT_TOKEN_DECIMAL } from 'config'
...
const getDateForBlock = async (currentBlock: number, block: number) => {
  ...
  if (currentBlock > block) {
    try {
      const { timestamp } = await publicClient({ chainId: ChainId.DNERO }).getBlock({ blockNumber: BigInt(block) })
  ...
  const secondsUntilStart = blocksUntilBlock * DNERO_BLOCK_TIME
...
export const processAuctionData = async (
  ...
  const currentBlock = currentBlockNumber || Number(await publicClient({ chainId: ChainId.DNERO }).getBlockNumber())
