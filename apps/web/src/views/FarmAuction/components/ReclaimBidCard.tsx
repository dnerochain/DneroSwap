...
import { Text, Heading, Card, CardHeader, CardBody, Flex, useToast } from '@dneroswap/uikit'
import { useTranslation } from '@dneroswap/localization'
import useApproveConfirmTransaction from 'hooks/useApproveConfirmTransaction'
import { useWDnero, useFarmAuctionContract } from 'hooks/useContract'
...
const ReclaimBidCard: React.FC<React.PropsWithChildren> = () => {
  ...
  const { isApproving, isApproved, isConfirming, handleApprove, handleConfirm } = useApproveConfirmTransaction({
    onRequiresApproval: async () => {
      return requiresApproval(wdneroContract, account, farmAuctionContract.address)
    },
    onApprove: () => {
      return callWithGasPrice(wdneroContract, 'approve', [farmAuctionContract.address, MaxUint256])
    },
