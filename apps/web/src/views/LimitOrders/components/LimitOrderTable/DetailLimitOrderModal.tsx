...
import { TransactionErrorContent, ConfirmationPendingContent } from '@dneroswap/widgets-internal'
...
export const DetailLimitOrderModal: React.FC<React.PropsWithChildren<DetailLimitOrderModalProps>> = ({
  ...
  const { isOpen, isExecuted, isExpired, isCancelled, isSubmissionPending, isCancellationPending, dneroScanUrls } =
    formattedOrder
  ...
  const orderDetails = (
    <>
      ...
      <LimitTradeInfoCard
        currentPriceExchangeRateText="0.002474 DTOKEN = 1 BUSD"
        currentPriceExchangeRateTextReversed="404.11169 BUSD = 1 DTOKEN"
      ...
      <Flex flexDirection="column">
        {isOpen || isExpired ? (
          <>
            <Button variant="primary" mt="16px" as="a" external href={formattedOrder.dneroScanUrls.created}>
              {t('View on DneroScan')}
          ...
          <Button variant="primary" mt="16px" as="a" external href={formattedOrder.dneroScanUrls.created}>
            {t('View order creation on DNEROScan')}
            <DneroScanIcon color="invertedContrast" ml="4px" />
          </Button>
        )}
        {isCancelled && dneroScanUrls.cancelled && (
          <Button variant="primary" mt="16px" as="a" external href={dneroScanUrls.cancelled}>
            {t('View order cancellation on DNEROScan')}
            <DneroScanIcon color="invertedContrast" ml="4px" />
          </Button>
        )}
        {isExecuted && dneroScanUrls.executed && (
          <Button variant="primary" mt="16px" as="a" external href={dneroScanUrls.executed}>
            {t('View order execution on DNEROScan')}
            <DneroScanIcon color="invertedContrast" ml="4px" />
          </Button>
        )}
      </Flex>
