...
import { Flex, Link, OpenNewIcon, Text, useMatchBreakpoints, Button, Svg, SvgProps } from '@dneroswap/uikit'
import { ASSET_CDN } from 'config/constants/endpoints'
import Image from 'next/legacy/image'
import { memo } from 'react'
import { styled } from 'styled-components'
import * as S from './Styled'
import { opdneroBunny, opdneroBg, opdneroBgMobile } from './images'

const pancakeSwapLogo = `${ASSET_CDN}/web/banners/ethXpancakeswap.png`

const OpDneroLogo = (props: SvgProps) => (
...
export const OpDneroBanner = memo(() => {
      ...
      <S.Inner>
        <S.LeftWrapper position="relative" style={{ zIndex: 3, justifyContent: isMobile ? 'flex-start' : 'center' }}>
          <Flex alignItems="center" mb={isMobile ? '0' : '0.5rem'} style={{ gap: isMobile ? 8 : 10 }}>
            ...
            />
            <Divider />
            <OpDneroLogo width={33} height={33} />
          </Flex>
          <Header>{isMobile ? t('opDNERO is LIVE!') : t('DneroSwap Now Live on opDNERO!')}</Header>
          {!isMobile && <SubTitle>{t('Swap and Provide Liquidity Now')}</SubTitle>}
          <Link
            style={{ textDecoration: 'none' }}
            external
            href="https://blog.pancakeswap.finance/articles/dneroswap-swap-v3-on-opDNERO"
          >
        ...
        <RightWrapper>
          {isDesktop ? (
            <>
              <Image src={opdneroBunny} alt="opdneroBunny" width={172} height={200} placeholder="blur" />
            </>
          ) : (
            <Image src={opdneroBunny} alt="opdneroBunny" width={151} height={176} placeholder="blur" />
          )}
          <BgWrapper>
            {isDesktop ? (
              <>
                <Image src={opdneroBg} alt="opdneroBg" width={1128} height={192} placeholder="blur" />
              </>
            ) : (
              <Image src={opdneroBgMobile} alt="opdneroBgMobile" width={358} height={181} placeholder="blur" />
            )}
          </BgWrapper>
        </RightWrapper>
      </S.Inner>
