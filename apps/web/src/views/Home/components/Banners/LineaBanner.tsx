...
import { Flex, Link, OpenNewIcon, Text, useMatchBreakpoints, Button, Svg } from '@dneroswap/uikit'
    ...
    >
      <S.Inner>
        <S.LeftWrapper position="relative" style={{ zIndex: 2, justifyContent: isMobile ? 'flex-start' : 'center' }}>
          ...
          <Header>{isMobile ? t('Linea is LIVE!') : t('DneroSwap Now Live on Linea!')}</Header>
          {!isMobile && <SubTitle>{t('Swap and Provide Liquidity on Linea now')}</SubTitle>}
          <Link
            style={{ textDecoration: 'none' }}
            external
            href="https://blog.pancakeswap.finance/articles/dneroswap-swap-v3-on-linea"
          >
