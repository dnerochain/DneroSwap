...
import { ArrowForwardIcon, Box, Button, Flex, Link, OpenNewIcon, Text, useMatchBreakpoints } from '@dneroswap/uikit'
import { NextLinkFromReactRouter } from '@dneroswap/widgets-internal'
import Image from 'next/legacy/image'
import { memo } from 'react'
import { keyframes, styled } from 'styled-components'
import * as S from './Styled'
import { vewdneroBg, vewdneroDesktopBunny, vewdneroMobileBg, vewdneroMobileBunny, vewdneroRuby, vewdneroTitle } from './images'
    ...
    >
      <S.Inner>
        <S.LeftWrapper position="relative" style={{ zIndex: 3, justifyContent: isMobile ? 'flex-start' : 'center' }}>
          ...
          <Header data-text={isMobile ? t('Introducing Gauges Voting and veWDNERO') : t('Your WDNERO, Your Voice')}>
            {isMobile ? t('Introducing Gauges Voting and veWDNERO') : t('Your WDNERO, Your Voice')}
          </Header>
          <Box>
            {isDesktop ? (
              <Flex mt="8px">
          ...
          {isDesktop ? (
            <Flex style={{ gap: isMobile ? 4 : 16 }}>
              <NextLinkFromReactRouter to="/wdnero-staking" prefetch={false}>
              ...
              <Link
                href="https://blog.pancakeswap.finance/articles/introducing-gauges-voting-and-ve-wdnero-your-ownership-of-wdnero-emissions"
                external
            ...
            <Link href="/wdnero-staking" external style={{ textDecoration: 'none' }}>
              <StyledButton>
                <Text
                  bold
                  color="invertedContrast"
        ...
        <RightWrapper>
          <Image src={vewdneroRuby} alt="vewdneroRuby" width={73.52} height={77.7} placeholder="blur" />
          {isMobile ? (
            <Image src={vewdneroMobileBunny} alt="vewdneroMobileBunny" width={161} height={177.7} placeholder="blur" />
          ) : (
            <Image src={vewdneroDesktopBunny} alt="vewdneroDesktopBunny" width={234.5} height={257.46} placeholder="blur" />
          )}
          <BgWrapper>
            {isMobile ? (
              <Image src={vewdneroMobileBg} alt="vewdneroBg" placeholder="blur" />
            ) : (
              <Image src={vewdneroBg} alt="vewdneroBg" width={1126} height={192} placeholder="blur" />
            )}
          </BgWrapper>
        </RightWrapper>
      </S.Inner>
...
export default memo(VeWDneroBanner)
