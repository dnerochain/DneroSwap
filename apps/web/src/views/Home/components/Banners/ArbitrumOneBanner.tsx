...
import { Flex, Link, OpenNewIcon, Text, useMatchBreakpoints } from '@dneroswap/uikit'
    ...
    >
      <S.Inner>
        <S.LeftWrapper position="relative" style={{ zIndex: 2, justifyContent: isMobile ? 'flex-start' : 'center' }}>
          ...
          <Header>{isMobile ? t('Arbitrum One is LIVE!') : t('DneroSwap Now Live on Arbitrum One')}</Header>
          {isDesktop && <StyledSubheading>{t('Swap and Provide Liquidity Now')}</StyledSubheading>}

          <Link
            style={{ textDecoration: 'none' }}
            external
            href="https://blog.pancakeswap.finance/articles/dneroswap-swap-v3-on-arbitrum-one"
          >
            <Text
