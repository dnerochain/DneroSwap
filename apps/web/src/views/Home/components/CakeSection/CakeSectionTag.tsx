...
export const WDneroPartnerWrapper = styled.div<{ $bgWidth: number }>`
  display: flex;
  justify-content: center;
  align-items: center;
  height: 40px;
...
export const WDneroSectionTag: React.FC<{ icon: React.ReactElement; text: string }> = ({ icon, text }) => {
...
export const WDneroPartnerTag: React.FC<{
  ...
  const [bgWidth, setBgWidth] = useState(() => textRef?.current?.offsetWidth ?? 0)
  return (
    <WDneroPartnerWrapper $bgWidth={bgWidth + 58}>
      {cloneElement(icon, { color: theme.isDark ? 'white' : 'black', width })}
      <Text fontWeight="400" ref={textRef}>
        {text}
      </Text>
    </WDneroPartnerWrapper>
  )
}
