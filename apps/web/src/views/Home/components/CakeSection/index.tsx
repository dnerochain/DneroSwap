...
import { Button, Flex, Link, OpenNewIcon, Text, useMatchBreakpoints } from '@dneroswap/uikit'
...
export const WDneroSectionMainBox = styled.div`
  position: relative;
  display: flex;
...
`
export const WDneroSectionLeftBox = styled.div`
  position: relative;
  display: flex;
  flex-grow: 1;
  flex-shrink: 0;
...
`
export const WDneroSectionRightBox = styled.div`
  position: relative;
  z-index: 1;
  display: flex;
  flex-grow: 1;
...
const WDneroLeftLine = styled.div`
  position: absolute;
  z-index: 2;
  height: 128px;
  width: 1px;
...
const WDneroRightLine = styled.div`
  position: absolute;
  height: 92px;
  width: 1px;
  left: 50%;
...
const WDneroRightBorderBox = styled.div`
  position: relative;
  ${borderBoxAnimation}
  &.show ${WDneroRightLine} {
    transform: scaleY(1);
    ${({ theme }) => theme.mediaQueries.lg} {
      transform: scaleX(1);
    }
...
`
const WDneroLeftBorderBox = styled.div`
  position: relative;
  z-index: 3;
  margin-top: 118px;
  ${({ theme }) => theme.mediaQueries.lg} {
...
`
const WDneroLeftBorder = styled.div`
  position: absolute;
  width: 100%;
  height: 1px;
  background: ${({ theme }) => theme.colors.secondary};
...
const WDneroRightBorder = styled.div`
  position: absolute;
  width: 100%;
  height: 1px;
  background: ${({ theme }) => theme.colors.secondary};
...
export const WDneroSectionCenterBox = styled.div`
  position: relative;
  z-index: 2;
  display: flex;
  margin-top: 20px;
...
`
const WDneroCanvas = styled.canvas`
  position: absolute;
  top: 50%;
  left: 50%;
...
const WDneroSection: React.FC = () => {
  ...
  const wdneroBoxRef = useRef<HTMLDivElement>(null)
  ...
  useLayoutEffect(() => {
    ...
    video.width = width
    video.src = `${ASSET_CDN}/web/landing/wdnero-alpha.webm`
    video.muted = true
  ...
  const { drawSequenceImage, playing } = useDrawSequenceImages(
    `${ASSET_CDN}/web/landing/wdnero-token-sequence`,
  ...
  return (
    ...
    >
      ...
      <Flex justifyContent="center">
        ...
        >
          {t(
            'Experience the power of community ownership, global governance, and explore infinite use cases within the DneroSwap ecosystem',
          )}
        </Text>
      </Flex>
      <Flex justifyContent="center" style={{ gap: 14 }}>
        <Link href="https://pancakeswap.finance/swap?outputCurrency=0x0e09fabb73bd3ade0a17ecc321fd13a19e81ce82&chainId=5647">
          <Button variant="primary">{t('Buy WDNERO')}</Button>
        </Link>
        <Link href="https://docs.pancakeswap.finance/governance-and-tokenomics/wdnero-tokenomics">
          <Button pl="0" endIcon={<OpenNewIcon color="primary" />} variant="text">
            {t('Learn')}
          </Button>
        </Link>
      </Flex>
      <WDneroSectionMainBox>
        <WDneroLeftLine ref={leftLineRef} className={played?.current ? 'show' : ''} />
        <WDneroSectionLeftBox>
          <WDneroLeftBorderBox ref={leftRef} className={played?.current ? 'show' : ''}>
            <WDneroLeftBorder />
            ...
            <EcoSystemTagOuterWrapper>
              <FeatureTagsWrapper direction={isMobile || isTablet ? 'right' : 'up'}>
                {ecosystemTagData.map((item) => (
                  <WDneroSectionTag key={item.text} icon={item.icon} text={item.text} />
                ))}
              </FeatureTagsWrapper>
            </EcoSystemTagOuterWrapper>
          </WDneroLeftBorderBox>
        </WDneroSectionLeftBox>
        <WDneroSectionCenterBox>
          <WDneroBox ref={wdneroBoxRef}>
            <WDneroCanvas width={width} height={height} ref={canvasRef} />
          </WDneroBox>
        </WDneroSectionCenterBox>
        <WDneroSectionRightBox>
          <WDneroRightBorderBox ref={rightRef} className={played?.current ? 'show' : ''}>
            <WDneroRightBorder />
            <WDneroRightLine />
            <Text textAlign="center" fontSize="40px" fontWeight="600" mb="20px">
              {t('Partners')}
            ...
            <PartnerTagOuterWrapper>
              <PartnerTagsWrapper direction={isMobile || isTablet ? 'right' : 'up'}>
                {partnerData.map((d) => (
                  <WDneroPartnerTag icon={d.icon} width={d.width} text={d.text} />
                ))}
              </PartnerTagsWrapper>
            </PartnerTagOuterWrapper>
          </WDneroRightBorderBox>
        </WDneroSectionRightBox>
      </WDneroSectionMainBox>
    </Flex>
  )
...
export default memo(WDneroSection)
