...
const HarvestCard: React.FC<React.PropsWithChildren<HarvestCardProps>> = ({ onHarvestStart, onHarvestEnd }) => {
  ...
  const earningsBusd = new BigNumber(farmEarningsSum).multipliedBy(wdneroPriceBusd)
  ...
  const hasWDneroPoolToCollect = numTotalToCollect - numFarmsToCollect > 0

  const earningsText = getEarningsText(numFarmsToCollect, hasWDneroPoolToCollect, earningsBusd, t)
