...
import { Button, Flex, Text, useMatchBreakpoints } from '@dneroswap/uikit'
...
const WDneroVideo = styled.video``
...
const Hero = () => {
  ...
  const wdneroVideoRef = useRef<HTMLVideoElement>(null)
  ...
  const { drawImage, isVideoPlaying } = useDrawCanvas(
    ...
    [starVideoRef, wdneroVideoRef, rock01VideoRef, rock02VideoRef, rock03VideoRef],
  )

  useLayoutEffect(() => {
    starVideoRef.current?.play()
    wdneroVideoRef.current?.play()
        ...
        >
          <BunnyWrapper>
            <WDneroBox>
              ...
              {!(isIOS || isMobile) && (
                <VideoWrapper>
                  <WDneroVideo ref={videoRef} width={width} autoPlay muted playsInline>
                    <source src={`${ASSET_CDN}/web/landing/bunnyv2.webm`} type="video/webm" />
                  </WDneroVideo>
                  <WDneroVideo ref={starVideoRef} width={width} autoPlay loop muted playsInline>
                    <source src={`${ASSET_CDN}/web/landing/star.webm`} type="video/webm" />
                  </WDneroVideo>
                  <WDneroVideo ref={wdneroVideoRef} width={width} autoPlay loop muted playsInline>
                    <source src={`${ASSET_CDN}/web/landing/hero-wdnero.webm`} type="video/webm" />
                  </WDneroVideo>
                  <WDneroVideo ref={rock01VideoRef} width={width} autoPlay loop muted playsInline>
                    <source src={`${ASSET_CDN}/web/landing/rock01.webm`} type="video/webm" />
                  </WDneroVideo>
                  <WDneroVideo ref={rock02VideoRef} width={width} autoPlay loop muted playsInline>
                    <source src={`${ASSET_CDN}/web/landing/rock02.webm`} type="video/webm" />
                  </WDneroVideo>
                  <WDneroVideo ref={rock03VideoRef} width={width} autoPlay loop muted playsInline>
