...
const BuyTicketsModal: React.FC<React.PropsWithChildren<BuyTicketsModalProps>> = ({
  ...
  const isWDneroBalanceInsufficient = wdneroBalance < pricePerTicket
  const maxBuyTickets = Math.min(Number(wdneroBalance / pricePerTicket), remainingTickets)
        ...
        <Flex flexDirection="column">
          <Box px="16px">
            ...
            <Flex mb="8px" justifyContent="space-between">
              ...
              <Text font-size="14px">{formatBigInt(pricePerTicket, 0)} WDNERO</Text>
            </Flex>
            <Flex mb="8px" justifyContent="space-between">
              <Text font-size="14px" color="textSubtle">
                {t('Your WDNERO Balance')}
              </Text>
              <Text font-size="14px" color={isWDneroBalanceInsufficient ? 'failure' : 'text'}>
                {formatBigInt(wdneroBalance, 3)} WDNERO
              </Text>
            </Flex>
            <Flex
              mb="8px"
            ...
            <Flex mb="25px" justifyContent="space-between">
              ...
              <Text font-size="14px" bold>
                {formatBigInt(totalCost, 0)} WDNERO
              </Text>
            </Flex>
          </Box>
          <Flex
          ...
          <Box px="16px">
            <Button
              disabled={isWDneroBalanceInsufficient}
              onClick={() => buyTicketCallBack({ ticketsNumber })}
              width="100%"
            >
              {isWDneroBalanceInsufficient ? t('Insufficient Balance') : t('Confirm')}
            </Button>
          </Box>
        </Flex>
      </ModalBody>
