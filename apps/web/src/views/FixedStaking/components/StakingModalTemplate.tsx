...
import { getFullDisplayBalance, getDecimalAmount } from '@dneroswap/utils/formatBalance'
...
import { CurrencyAmount, Percent, Currency } from '@dneroswap/sdk'
...
export function StakingModalTemplate({
  ...
  const [useDTOKEN, toggleUseDTOKEN] = useState(false)
  ...
  const isWDTOKEN = nativeToken.wrapped.equals(positionStakingToken)

  const enableNative = useNative && useDTOKEN
  ...
  const { balance: dtokenBalance } = useGetDTokenBalance()
  ...
  const stakingTokenBalance = useMemo(
    () => (enableNative ? new BigNumber(dtokenBalance?.toString()) : positionStakingTokenBalance),
    [dtokenBalance, enableNative, positionStakingTokenBalance],
  )
        ...
        <>
          <Flex ml="auto" alignItems="center" justifyContent="space-between" width="100%">
            ...
            <Toggle
              ...
              scale="sm"
              checked={useDTOKEN}
              onChange={() => toggleUseDTOKEN((prev) => !prev)}
            />
          </Flex>

          {useDTOKEN ? (
            <Message variant="warning" my="8px">
              <MessageText>You will receive WDTOKEN if you withdraw your staked DTOKEN.</MessageText>
            </Message>
          ) : null}
        </>
      ) : null}
