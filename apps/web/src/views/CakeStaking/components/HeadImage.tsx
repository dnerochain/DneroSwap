...
import { AtomBox, Box, Button, HelpIcon, Link, Svg, SvgProps, Text, useMatchBreakpoints } from '@dneroswap/uikit'
import useTheme from 'hooks/useTheme'
import styled from 'styled-components'
import { useWDneroLockStatus } from '../hooks/useVeWDneroUserInfo'
import { WDneroLockStatus } from '../types'

const SPEECH_BUBBLE_ID = 'wdnero-staking__speech-bubble'
...
export const NewWDneroStakingCard: React.FC = () => {
      ...
      >
        <SpeechBubbleBox display="flex">
          <Button variant="subtle" endIcon={<HelpIcon color="invertedContrast" width="24px" />}>
            {t('New to WDNERO Staking')}
          </Button>
          <SpeechBubble width="16px" height="16px" />
        </SpeechBubbleBox>
      </Link>
      <img src="/images/wdnero-staking/new-staking-bunny.png" alt="new-staking-bunny" width="138px" />
    </AtomBox>
  )
}
...
export const HeadBunny = () => {
  ...
  if (status === WDneroLockStatus.Locking) {
    return (
      <Link
        external
        href="https://docs.pancakeswap.finance/products/vewdnero/how-to-get-vewdnero"
  ...
  return <NewWDneroStakingCard />
}

export const MobileHeadBunny = () => {
  return (
    <Box position="relative">
      <StyleMobileBunny src="/images/wdnero-staking/new-staking-bunny.png" alt="new-staking-bunny" />
    </Box>
  )
}
