...
import { getChainName } from '@dneroswap/chains'
...
const getFarmConfig = async (chainId: number) => {
  const config = await import(`@dneroswap/farms/constants/${getChainName(chainId)}`)
  return config
}
        ...
        <>
          <Flex justifyContent="space-between" mb="16px" flexDirection={['column', 'column', 'row']}>
            ...
            <Flex justifyContent={[null, null, 'flex-end']} mt={['8px', '8px', 0]}>
              <ScanLink
                useDneroCoinFallback={ChainLinkSupportChains.includes(multiChainId[chainName])}
                mr="8px"
