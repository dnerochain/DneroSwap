...
import { Ifo, PoolIds, wdneroDTokenLpToken } from '@dneroswap/ifos'
...
import { IWDneroTips } from './IWDneroTips'
...
const CommitTokenSection: React.FC<React.PropsWithChildren<TokenSectionProps & { commitToken: Token }>> = ({
  ...
  if (commitToken.equals(wdneroDTokenLpToken)) {
    return <TokenSection primaryToken={dneroTokens.wdnero} secondaryToken={dneroTokens.wdtoken} {...props} />
  }
...
const IfoCardTokens: React.FC<React.PropsWithChildren<IfoCardTokensProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough WDNERO to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your WDNERO.',
    ),
    { placement: 'bottom' },
  )
  ...
  const renderTokenSection = () => {
    ...
    if (
      ...
      message = <IWDneroTips ifoId={ifo.id} ifoChainId={ifo.chainId} ifoAddress={ifo.address} />
    }
            ...
            (ifov31Msg || (
              <>
                <Text textAlign="center" fontSize="14px">
                  {t('To participate in the next IFO, lock some WDNERO in the fixed-term staking WDNERO pool!')}
                </Text>
