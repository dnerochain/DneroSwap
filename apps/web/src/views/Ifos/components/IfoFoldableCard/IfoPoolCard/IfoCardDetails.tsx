...
import { Text, Flex, Box, Skeleton, TooltipText, useTooltip, IfoSkeletonCardDetails } from '@dneroswap/uikit'
import { PublicIfoData, WalletIfoData } from 'views/Ifos/types'
import { useTranslation } from '@dneroswap/localization'
import { Ifo, PoolIds } from '@dneroswap/ifos'
import { BIG_ONE_HUNDRED } from '@dneroswap/utils/bigNumber'
import { getBalanceNumber, formatNumber } from '@dneroswap/utils/formatBalance'
import { useStablecoinPrice } from 'hooks/useBUSDPrice'
import { DAY_IN_SECONDS } from '@dneroswap/utils/getTimePeriods'
...
const MaxTokenEntry = ({
  ...
  const wdnero = WDNERO[ifo.chainId]
  const isCurrencyWDnero = cake && ifo.currency.wrapped?.equals(cake)
  ...
  const basicTooltipContent =
    ifo.version >= 3.1 && !basicSale
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of WDNERO up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max WDNERO entry is capped by minimum between your average WDNERO balance in the iWDNERO, or the pool’s hard cap. To increase the max entry, Stake more WDNERO into the iWDNERO',
        )

  const unlimitedToolipContent =
    ifo.version >= 3.1 ? (
      <Box>
        <Text display="inline">{t('For the public sale, Max WDNERO entry is capped by')} </Text>
        <Text bold display="inline">
          {t('the number of iWDNERO.')}{' '}
        </Text>
        <Text display="inline">
          {t('Lock more WDNERO for longer durations to increase the maximum number of WDNERO you can commit to the sale.')}
        </Text>
      </Box>
    ) : (
      t(
        'For the unlimited sale, Max WDNERO entry is capped by your average WDNERO balance in the iWDnero. To increase the max entry, Stake more WDNERO into the iWDnero',
      )
    )
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyWDnero ? t('Max. WDNERO entry') : t('Max. token entry')
        ...
        value={
          <Text small textAlign="right" color={maxToken > 0 ? 'text' : 'failure'}>
            {`${formatNumber(maxToken, 3, 3)} ${
              !isCurrencyWDnero ? ifo.currency.symbol : ''
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {raisingTokenToBurn && <FooterEntry label={t('WDNERO to burn:')} value={raisingTokenToBurn} />}
          <FooterEntry
