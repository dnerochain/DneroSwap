...
import { Text, Flex, AccountIcon, TeamBattleIcon, Box, useTooltip, LinkExternal } from '@dneroswap/uikit'
...
const NotOkNFT = ({ admissionProfile }) => {
  ...
  const keyword = '%Dneroswap Squad NFT%'

  const rawText = t(`Set %Dneroswap Squad NFT% as Dneroswap Profile avatar`)
...
const NotOkProfilePoints = ({ pointThreshold }) => {
  ...
  const keyword = '%Dneroswap Profile%'

  const rawText = t(`Reach %point% or more %Dneroswap Profile% points`, { point: pointThreshold })
