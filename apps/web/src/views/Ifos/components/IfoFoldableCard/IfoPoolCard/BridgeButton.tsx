...
import { Button, useModalV2, Loading } from '@dneroswap/uikit'
...
import { BRIDGE_STATE, useBridgeIWDnero } from '../../../hooks/useBridgeIWDnero'
import { BridgeIWDneroModal } from './BridgeIWDneroModal'

type Props = {
  ifoId: string

  ifoChainId: ChainId
  // The amount of iwdnero on source chain
  iwdnero?: CurrencyAmount<Currency>
  // The amount of iwdnero on destination chain
  dstIcake?: CurrencyAmount<Currency>

  buttonVisible?: boolean
} & SpaceProps

export function BridgeButton({ ifoChainId, iwdnero, dstIcake, buttonVisible = true, ifoId, ...props }: Props) {
  ...
  const { state, bridge, isBridging, isBridged, clearBridgeHistory } = useBridgeIWDnero({
    ifoId,
    iwdnero,
    dstIcake,
  ...
  return (
    <>
      ...
      />
      {buttonVisible && (
        <Button width="100%" id="bridge-iwdnero" disabled={loading} onClick={onBridgeClick} {...props}>
