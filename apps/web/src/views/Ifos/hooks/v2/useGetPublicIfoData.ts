...
import { Ifo, IfoStatus } from '@dneroswap/ifos'
...
// https://github.com/pancakeswap/dneroswap-contracts/blob/master/projects/ifo/contracts/IFOV2.sol#L431
// 1,000,000,000 / 100
...
const useGetPublicIfoData = (ifo: Ifo): PublicIfoData => {
  const { address } = ifo
  const wdneroPriceUsd = useWDneroPrice()
  const lpTokenPriceInUsd = useLpTokenPrice(ifo.currency.symbol)
  const currencyPriceInUSD = ifo.currency === dneroTokens.wdnero ? wdneroPriceUsd : lpTokenPriceInUsd
      ...
      setState(
        (prev) =>
          ({
            ...prev,
            isInitialized: true,
            secondsUntilEnd: blocksRemaining * DNERO_BLOCK_TIME,
            secondsUntilStart: (startBlockNum - currentBlock) * DNERO_BLOCK_TIME,
            poolBasic: {
              ...poolBasicFormatted,
              taxRate: 0,
            },
