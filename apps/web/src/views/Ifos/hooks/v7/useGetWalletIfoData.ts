...
import { Ifo, PoolIds, ifoV7ABI } from '@dneroswap/ifos'
...
const useGetWalletIfoData = (ifo: Ifo): WalletIfoData => {
  ...
  const fetchIfoData = useCallback(async () => {
    ...
    const dneroClient = publicClient({ chainId })

    const [userInfo, amounts] = await dneroClient.multicall({
      contracts: [
        {
          address,
          abi: ifoV7ABI,
    ...
    if (version >= 3.2) {
      const [basicIdDataResult, unlimitedIdDataResult] = await dneroClient.multicall({
        contracts: [
          {
            address,
            abi: ifoV7ABI,
    ...
    dispatch(fetchWDneroVaultUserData({ account, chainId: sourceChain }))
