...
const SORT_FIELD = {
  createdAt: 'createdAt',
  volumeDTOKEN: 'totalVolumeDTOKEN',
              ...
              <Card style={{ overflowX: 'auto' }} mb="32px">
                <Table>
                  <thead>
                    <tr>
                      ...
                      <Th
                        textAlign="left"
                        style={{ cursor: 'pointer' }}
                        onClick={() => handleSort(SORT_FIELD.volumeDTOKEN)}
                      >
                        {t('Volume')}
                        {arrow(SORT_FIELD.volumeDTOKEN)}
                      </Th>
                      <Th
                        textAlign="left"
                        style={{ cursor: 'pointer' }}
                  ...
                  <tbody>
                    {sortedCollections
                      .map((collection) => {
                        const volume = collection.totalVolumeDTOKEN
                          ? parseFloat(collection.totalVolumeDTOKEN).toLocaleString(undefined, {
                        ...
                        return (
                          <tr key={collection.address} data-test="nft-collection-row">
                            ...
                            <Td>
                              <Flex alignItems="center">
                                {volume}
                                <DTokenUsdtPairTokenIcon ml="8px" />
                              </Flex>
                            </Td>
                            <Td>
                ...
                {sortedCollections.slice(ITEMS_PER_PAGE * (page - 1), page * ITEMS_PER_PAGE).map((collection) => {
                  return (
                    <CollectionCardWithVolume
                      ...
                      volume={collection.totalVolumeDTOKEN ? parseFloat(collection.totalVolumeDTOKEN) : 0}
                    />
                  )
                })}
              </Grid>
