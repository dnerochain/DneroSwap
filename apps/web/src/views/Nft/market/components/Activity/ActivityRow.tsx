...
const ActivityRow: React.FC<React.PropsWithChildren<ActivityRowProps>> = ({
  ...
  const isPBCollection = nft ? safeGetAddress(nft.collectionAddress) === safeGetAddress(dneroswapBunniesAddress) : false
  const tokenId =
    nft && isPBCollection
  ...
  return (
    <tr {...((isXs || isSm) && onClickProp)} data-test="nft-activity-row">
      ...
      <Td>
        ...
        {isXs || isSm ? <ActivityPrice price={priceAsFloat} dtokenBusdPrice={dtokenBusdPrice} /> : null}
      </Td>
      {isXs || isSm ? null : (
        <>
          <Td>
            <ActivityPrice price={priceAsFloat} dtokenBusdPrice={dtokenBusdPrice} />
          </Td>
          {isUserActivity ? (
            <Td>
      ...
      {isXs || isSm ? null : (
        <Td>
          <IconButton as={Link} external href={getBlockExploreLink(activity.tx, 'transaction', chainId)}>
            <DneroScanIcon color="textSubtle" width="18px" />
          </IconButton>
        </Td>
      )}
    </tr>
