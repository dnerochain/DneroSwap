...
import { Modal, Box, Flex, Text, BinanceIcon, Input } from '@dneroswap/uikit'
...
interface DTokenAmountCellProps {
  dtokenAmount: number
}

export const DTokenAmountCell: React.FC<React.PropsWithChildren<DTokenAmountCellProps>> = ({ dtokenAmount }) => {
  const dtokenBusdPrice = useDTOKENPrice()
  if (!dtokenAmount || dtokenAmount === 0) {
    return (
  ...
  const usdAmount = dtokenBusdPrice.multipliedBy(dtokenAmount).toNumber()
  return (
    <Flex alignItems="center" justifyContent="flex-end">
      <BinanceIcon width={16} height={16} mr="4px" />
      <Text bold mr="4px">{`${dtokenAmount.toLocaleString(undefined, {
...
export const FeeAmountCell: React.FC<React.PropsWithChildren<FeeAmountCellProps>> = ({
  ...
  const feeAmount = dtokenAmount * totalFeeAsDecimal
  return (
    <Flex alignItems="center" justifyContent="flex-end">
      <BinanceIcon width={16} height={16} mr="4px" />
