...
import { Flex, Grid, Box, Text, Button, BinanceIcon, ErrorIcon, useTooltip, Skeleton } from '@dneroswap/uikit'
...
import { GreyedOutContainer, DTokenAmountCell, RightAlignedInput, FeeAmountCell } from './styles'
...
const SetPriceStage: React.FC<React.PropsWithChildren<SetPriceStageProps>> = ({
  ...
  const priceInUsd = dtokenBusdPrice.multipliedBy(priceAsFloat).toNumber()
  ...
  const { tooltip, tooltipVisible, targetRef } = useTooltip(
    <>
      <Text>
        {t(
          'When selling NFTs from this collection, a portion of the DTOKEN paid will be diverted before reaching the seller:',
        )}
      </Text>
      {creatorFeeAsNumber > 0 && (
        <Text>{t('%percentage%% royalties to the collection owner', { percentage: creatorFee })}</Text>
      )}
      <Text>{t('%percentage%% trading fee will be used to buy & burn WDNERO', { percentage: tradingFee })}</Text>
    </>,
  )
    ...
    <>
      ...
      <GreyedOutContainer>
        ...
        {priceIsOutOfRange && (
          <Text fontSize="12px" color="failure">
            {t('Allowed price range is between %minPrice% and %maxPrice% WDTOKEN', {
              minPrice: MIN_PRICE,
              maxPrice: MAX_PRICE,
            })}
          </Text>
        ...
        <Flex justifyContent="space-between" alignItems="center" mt="16px">
          ...
          {Number.isFinite(creatorFeeAsNumber) && Number.isFinite(tradingFeeAsNumber) ? (
            <FeeAmountCell dtokenAmount={priceAsFloat} creatorFee={creatorFeeAsNumber} tradingFee={tradingFeeAsNumber} />
          ) : (
            <Skeleton width={40} />
          )}
        </Flex>
        {lowestPrice && (
          <Flex justifyContent="space-between" alignItems="center" mt="16px">
            ...
            <DTokenAmountCell dtokenAmount={lowestPrice} />
          </Flex>
        )}
      </GreyedOutContainer>
      <Grid gridTemplateColumns="32px 1fr" p="16px" maxWidth="360px">
        ...
        <Box>
          ...
          <Text small color="textSubtle">
            {t('Sales are in WDTOKEN. You can swap WDTOKEN to DTOKEN 1:1 for free with DneroSwap.')}
          </Text>
        </Box>
      </Grid>
      <Divider />
