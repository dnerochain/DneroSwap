...
const Header: React.FC<React.PropsWithChildren<HeaderProps>> = ({ collection }) => {
  ...
  const { totalSupply, numberTokensListed, totalVolumeDTOKEN, banner, avatar } = collection
  const { t } = useTranslation()

  const volume = totalVolumeDTOKEN
    ? parseFloat(totalVolumeDTOKEN).toLocaleString(undefined, {
      ...
      <MarketPageHeader>
        ...
        >
          <StatBox>
            ...
            <StatBoxItem title={t('Vol. (%symbol%)', { symbol: 'DTOKEN' })} stat={volume} />
          </StatBox>
        </MarketPageTitle>
      </MarketPageHeader>
      <Container>
