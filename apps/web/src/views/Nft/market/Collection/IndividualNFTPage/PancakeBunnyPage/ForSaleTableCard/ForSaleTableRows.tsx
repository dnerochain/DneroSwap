...
import { Button, Grid, Text, Flex, Box, BinanceIcon, useModal, Skeleton } from '@dneroswap/uikit'
import { formatNumber } from '@dneroswap/utils/formatBalance'
import { ContextApi, useTranslation } from '@dneroswap/localization'
...
const Row: React.FC<React.PropsWithChildren<RowProps>> = ({ t, nft, dtokenBusdPrice, account, onSuccessSale }) => {
  const priceInUsd = dtokenBusdPrice.multipliedBy(parseFloat(nft?.marketData?.currentAskPrice)).toNumber()
    ...
    <OwnersTableRow>
      {nftsForSale.map((nft) => (
        <Row
          ...
          t={t}
          nft={nft}
          dtokenBusdPrice={dtokenBusdPrice}
          account={account}
