...
import { ArrowBackIcon, ArrowForwardIcon, Box, IconButton, Text, Flex, useMatchBreakpoints } from '@dneroswap/uikit'
...
const MoreFromThisCollection: React.FC<React.PropsWithChildren<MoreFromThisCollectionProps>> = ({
  ...
  const allDneroswapBunnyNfts = useAllDneroswapBunnyNfts(collectionAddress)
  ...
  const nftsToShow = useMemo(() => {
    let shuffled = shuffle(
      allDneroswapBunnyNfts
        ? allDneroswapBunnyNfts.filter((nft) => nft.name !== currentTokenName)
        : collectionNfts?.filter((nft) => nft.name !== currentTokenName && nft.marketData?.isTradable),
    )

    if (isPBCollection) {
      // DneroswapBunnies should display 1 card per bunny id
  ...
  }, [allDneroswapBunnyNfts, collectionNfts, currentTokenName, isPBCollection])
