...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text, useToast } from '@dneroswap/uikit'
...
const PauseProfilePage: React.FC<React.PropsWithChildren<PauseProfilePageProps>> = ({ onDismiss, onSuccess }) => {
  ...
  const {
    costs: { numberWDneroToReactivate },
  } = useGetProfileCosts()
  const { t } = useTranslation()
  const dneroswapProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(dneroswapProfileContract, 'pauseProfile')
    })
    if (receipt?.status) {
      // Re-fetch profile
      refreshProfile()
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {t('Cost to reactivate in the future: %cost% WDNERO', { cost: formatBigInt(numberWDneroToReactivate) })}
      </Text>
