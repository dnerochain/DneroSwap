...
import { Box, Button, InjectedModalProps, Skeleton, Text, useToast } from '@dneroswap/uikit'
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        ...
        return approvedAddress !== getDneroswapProfileAddress()
      },
      onApprove: () => {
        const contract = getErc721Contract(selectedNft.collectionAddress, signer)

        return callWithGasPrice(contract, 'approve', [getDneroswapProfileAddress(), selectedNft.tokenId])
      },
      onConfirm: () => {
        if (!profile.isActive) {
    ...
    <>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {t('Make sure you have a Dneroswap Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
