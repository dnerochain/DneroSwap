...
import { Button, Flex, HelpIcon, useTooltip } from '@dneroswap/uikit'
...
const ButtonMenu: React.FC<React.PropsWithChildren<ButtonMenuProps>> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Your chance of winning is proportional to the WDNERO you deposit relative to the total WDNERO deposit for Pottery. Currently, there is a cap to the total WDNERO deposit size during the beta release.',
    ),
    {
      placement: 'top-end',
      tooltipOffset: [20, 10],
  ...
  return (
    <Flex justifyContent="space-between" mt="8px">
      ...
      <Button
        ...
        onClick={() => setPrincipalFromUSDValue(getBalanceNumber(stakingTokenBalance.times(wdneroPrice)).toString())}
      >
        {t('My Balance')}
      </Button>
      <span ref={targetRef}>
