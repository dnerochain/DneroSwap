...
const useWinRateCalculator = ({ wdneroPrice, totalSupply }: WinRateCalculatorProps) => {
  ...
  const totalLockValueAsUSD = useMemo(() => {
    return new BigNumber(totalLockValue).times(wdneroPrice).toNumber()
  }, [totalLockValue, wdneroPrice])
  ...
  const setPrincipalFromTokenValue = (amount: string) => {
    const principalAsUsdBN = new BigNumber(amount).times(wdneroPrice)
  ...
  const setPrincipalFromUSDValue = (amount: string) => {
    const principalAsTokenBN = new BigNumber(amount).div(wdneroPrice)
  ...
  const setTargetWinRate = (percentage: string) => {
    const wdneroAmount = new BigNumber(percentage).dividedBy(100).times(totalLockValue)
    const principalAsToken = wdneroAmount.gt(0) ? wdneroAmount.toFixed(TOKEN_PRECISION) : '0.00'
    const principalAsUsdBN = new BigNumber(wdneroAmount).times(wdneroPrice)
    const principalAsUsdString = principalAsUsdBN.gt(0) ? principalAsUsdBN.toFixed(USD_PRECISION) : '0.00'
