...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.DTOKEN, PredictionSupportedSymbol.WDNERO].includes(initToken)) {
      return initToken
    }

    return PredictionSupportedSymbol.WDNERO
  })

  useEffect(() => {
    const upperToken = _toUpper(token as string) as PredictionSupportedSymbol

    if ([PredictionSupportedSymbol.DTOKEN, PredictionSupportedSymbol.WDNERO].includes(upperToken)) {
