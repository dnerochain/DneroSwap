...
import { Box, Flex, FlexProps, Skeleton, Text } from '@dneroswap/uikit'
...
import { formatDToken, formatUsd } from '../History/helpers'
  ...
  return (
    <Row height="18px" {...props}>
      ...
      <Flex alignItems="center">
        ...
        <Text mx="4px">|</Text>
        <Text fontSize="12px" lineHeight="18px">{`${formatDToken(amount, displayedDecimals)} ${token.symbol}`}</Text>
      </Flex>
    </Row>
  )
}
...
const getPrizePoolAmountHistory = (totalAmount: PrizePoolHistoryRowProps['totalAmount'], displayedDecimals: number) => {
  ...
  return formatDToken(totalAmount, displayedDecimals)
}
