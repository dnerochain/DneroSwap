...
import { useGetDTokenBalance, useDNEROWDneroBalance } from 'hooks/useTokenBalance'
...
const getButtonProps = (value: bigint, dtokenBalance: bigint, minBetAmountBalance: bigint) => {
  const hasSufficientBalance = () => {
    if (value > 0) {
      return value <= dtokenBalance
    }
    return dtokenBalance > 0
  }
...
const TOKEN_BALANCE_CONFIG = {
  DTOKEN: useGetDTokenBalance,
  WDNERO: useDNEROWDneroBalance,
} as const
...
const SetPositionCard: React.FC<React.PropsWithChildren<SetPositionCardProps>> = ({
  ...
  const { setLastUpdated, allowance } = useWDneroApprovalStatus(token.symbol === 'WDNERO' ? predictionsAddress : null)
  const { handleApprove, pendingTx } = useWDneroApprove(
    setLastUpdated,
    predictionsAddress,
    t('You can now start prediction'),
  )

  const { balance: dtokenBalance } = useTokenBalance()

  const maxBalance = useMemo(() => {
    return dtokenBalance > dust ? dtokenBalance - dust : 0n
  }, [dtokenBalance])
  const balanceDisplay = formatBigInt(dtokenBalance)

  const valueAsBn = getValueAsEthersBn(value)
  const showFieldWarning = account && valueAsBn > 0n && errorMessage !== null

  // DTOKEN prediction doesn't need approval
  const doesWDneroApprovePrediction = token.symbol === 'DTOKEN' || allowance.gte(valueAsBn.toString())
  ...
  const handleEnterPosition = async () => {
    ...
    const args = token.symbol === 'WDNERO' ? [epoch, valueAsBn.toString()] : [epoch]
