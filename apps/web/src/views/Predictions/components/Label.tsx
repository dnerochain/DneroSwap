import { Box, CoinSwitcher, Flex, PocketWatchIcon, Text, CloseIcon } from '@dneroswap/uikit'
...
export const PricePairLabel: React.FC<React.PropsWithChildren> = () => {
  ...
  const onTokenSwitch = useCallback(() => {
    if (router.query.token === PredictionSupportedSymbol.WDNERO) {
      router.query.token = PredictionSupportedSymbol.DTOKEN
    } else if (router.query.token === undefined && token.symbol === PredictionSupportedSymbol.WDNERO) {
      router.query.token = PredictionSupportedSymbol.DTOKEN
    } else if (router.query.token === undefined && token.symbol === PredictionSupportedSymbol.DTOKEN) {
      router.query.token = PredictionSupportedSymbol.WDNERO
  ...
  return (
    <>
      <Box pl={['20px', '20px', '20px', '20px', '40px']} position="relative" display="inline-block">
        ...
        <CoinSwitcher
          isDefaultDToken={router.query.token === 'DTOKEN' || (router.query.token === undefined && token.symbol === 'DTOKEN')}
