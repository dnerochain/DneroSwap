...
const HistoricalBet: React.FC<React.PropsWithChildren<BetProps>> = ({ bet }) => {
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <>
      ...
      <Text>
        {t(
          'The Locked Price & Closed Price are exactly the same (within 8 decimals), so neither side wins. All funds entered into UP and DOWN positions will go to the weekly WDNERO burn.',
        )}
      </Text>
    </>,
    { placement: 'top' },
      ...
      <>
        ...
        <Text bold color={resultTextColor} lineHeight={1}>
          {roundResult === Result.CANCELED ? (
            ...
            `${resultTextPrefix}${formatDToken(payout, displayedDecimals)}`
          )}
        </Text>
      </>
    )
