...
import { Box, Flex, Heading, Text, Button, Link, DneroScanIcon } from '@dneroswap/uikit'
...
import { formatDToken, getMultiplier, getNetPayout } from '../helpers'
...
const PnlTab: React.FC<React.PropsWithChildren<PnlTabProps>> = ({ hasBetHistory, bets }) => {
  ...
  const avgDTokenWonPerRound = netResultAmount / summary.entered.rounds
  const avgDTokenWonIsPositive = avgDTokenWonPerRound > 0
  ...
  const avgDTokenWonInUsd = tokenPrice.multipliedBy(avgDTokenWonPerRound).toNumber()
  const avgDTokenWonInUsdDisplay = !Number.isNaN(avgDTokenWonInUsd) ? `~${avgDTokenWonInUsd.toFixed(2)}` : '~$0.00'
  ...
  return hasBetHistory ? (
    <Box p="16px">
      ...
      <Flex>
        ...
        <Flex flexDirection="column" justifyContent="center" pl="24px">
          ...
          <Text bold fontSize="24px" lineHeight="1" color={netResultIsPositive ? 'success' : 'failure'}>
            {`${netResultIsPositive ? '+' : ''}${formatDToken(netResultAmount, displayedDecimals)} ${token.symbol}`}
          </Text>
      ...
      <Box pl="8px">
        ...
        <Text bold color={avgDTokenWonIsPositive ? 'success' : 'failure'}>
          {`${avgDTokenWonIsPositive ? '+' : ''}${formatDToken(avgDTokenWonPerRound, displayedDecimals)} ${token.symbol}`}
        </Text>
        <Text small color="textSubtle">
          {avgDTokenWonInUsdDisplay}
        </Text>
        ...
        {hasBestRound && (
          <>
            ...
            <Flex alignItems="flex-end">
              <Text bold color="success">{`+${formatDToken(summary.won.bestRound.payout, displayedDecimals)} ${
                token.symbol
              }`}</Text>
        ...
        <Text bold>{`${formatDToken(avgPositionEntered, displayedDecimals)} ${token.symbol}`}</Text>
        <Text small color="textSubtle">
        ...
        <Flex justifyContent="center" mt="24px">
          <Link href={`${getBlockExploreLink(account, 'address')}#internaltx`} mb="16px" external>
            <Button mt="8px" width="100%">
              ...
              <DneroScanIcon color="invertedContrast" ml="4px" />
            </Button>
          </Link>
        </Flex>
      </Box>
