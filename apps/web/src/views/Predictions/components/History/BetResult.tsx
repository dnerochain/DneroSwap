...
import { Box, Flex, Heading, Text, PrizeIcon, BlockIcon, useTooltip, InfoIcon, ScanLink } from '@dneroswap/uikit'
...
import { formatDToken, getNetPayout } from './helpers'
    ...
    <>
      ...
      <StyledBetResult>
        ...
        <Flex alignItems="center" justifyContent="space-between" mb="16px">
          ...
          <Text>{`${formatDToken(bet.amount, displayedDecimals)} ${token.symbol}`}</Text>
        </Flex>
        <Flex alignItems="start" justifyContent="space-between">
          <Text bold>{isWinner ? t('Your winnings') : t('Your Result')}:</Text>
          <Box style={{ textAlign: 'right' }}>
            <Text bold color={resultColor}>{`${isWinner ? '+' : '-'}${formatDToken(payout, displayedDecimals)} ${
              token.symbol
            }`}</Text>
          ...
          <>
            ...
            <Flex alignItems="start" justifyContent="space-between">
              ...
              <Flex justifyContent="end">
                <Text fontSize="14px" color="textSubtle">{`${formatDToken(returned, displayedDecimals)} ${
                  token.symbol
                }`}</Text>
                <span ref={targetRef}>
