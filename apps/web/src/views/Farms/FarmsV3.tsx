...
import { FarmWidget, NextLinkFromReactRouter } from '@dneroswap/widgets-internal'
...
import { BIG_ONE, BIG_ZERO } from '@dneroswap/utils/bigNumber'
import Table from './components/FarmTable/FarmTable'
import { FarmTypesFilter } from './components/FarmTypesFilter'
import { BWDneroBoosterCard } from './components/YieldBooster/components/bWDneroV3/BWDneroBoosterCard'
...
const Farms: React.FC<React.PropsWithChildren> = ({ children }) => {
  ...
  const { data: farmsV2, userDataLoaded: v2UserDataLoaded, poolLength: v2PoolLength, regularWDneroPerBlock } = useFarms()
  const {
  ...
  const farmsList = useCallback(
    (farmsToDisplay: V2AndV3Farms): V2StakeValueAndV3Farm[] => {
      const farmsToDisplayWithAPR: any = farmsToDisplay.map((farm) => {
        ...
        const { wdneroRewardsApr, lpRewardsApr } =
          isActive && chainId
            ? getFarmApr(
                chainId,
            ...
            : { wdneroRewardsApr: 0, lpRewardsApr: 0 }

        return { ...farm, apr: wdneroRewardsApr, lpRewardsApr, liquidity: totalLiquidity }
      })

      return filterFarmsByQuery(farmsToDisplayWithAPR, query)
    },
    [query, isActive, chainId, wdneroPrice, regularWDneroPerBlock, mockApr],
  )
  ...
  const chosenFarmsMemoized = useMemo(() => {
    const sortFarms = (farms: V2StakeValueAndV3Farm[]): V2StakeValueAndV3Farm[] => {
      switch (sortOption) {
        case 'apr':
          return orderBy(farms, (farm) => (farm.version === 3 ? Number(farm.wdneroApr) : farm.apr ?? 0), 'desc')
        case 'multiplier':
        ...
        case 'earned':
          return orderBy(
            ...
            (farm) => {
              ...
              const totalEarned = Object.values(farm.pendingWDneroByTokenIds)
                .reduce((a, b) => a + b, 0n)
                .toString()
      ...
      <PageHeader>
        <Flex flexDirection="column">
          <FarmFlexWrapper justifyContent="space-between">
            ...
            {(chainId === ChainId.DNERO || chainId === ChainId.DNERO_TESTNET) && (
              <Box>
                <BWDneroBoosterCard />
              </Box>
            )}
      ...
      <Page>
        ...
        {!isLoading && // FarmV3 initial data will be slower, wait for it loads for now to prevent showing the v2 farm from config and then v3 pop up later
          (viewMode === ViewMode.TABLE ? (
            <Table farms={chosenFarmsMemoized} wdneroPrice={wdneroPrice} userDataReady={userDataReady} />
          ) : (
        ...
        <StyledImage src="/images/decorations/3dpan.png" alt="Dneroswap illustration" width={120} height={103} />
