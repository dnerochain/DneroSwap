...
import { Box, Card, ExpandableSectionButton, Flex, Text, TooltipText, useModalV2, useTooltip } from '@dneroswap/uikit'
...
import { StatusView } from '../../YieldBooster/components/bWDneroV3/StatusView'
import { useUserBoostedPoolsTokenId } from '../../YieldBooster/hooks/bWDneroV3/useBWDneroV3Info'
import { BoostStatus, useBoostStatus } from '../../YieldBooster/hooks/bWDneroV3/useBoostStatus'
import { useIsSomePositionBoosted } from '../../YieldBooster/hooks/bWDneroV3/useIsSomePositionBoosted'
...
export const FarmV3Card: React.FC<React.PropsWithChildren<FarmCardProps>> = ({ farm, removed, account }) => {
  ...
  const { totalMultipliers, getFarmWDneroPerSecond } = useFarmV3Multiplier()

  const farmWDneroPerSecond = getFarmWDneroPerSecond(farm.poolWeight)

  const lpLabel = farm.lpSymbol && farm.lpSymbol.replace(/pancake/gi, '')
  const earnLabel = t('WDNERO + Fees')
  const { lpAddress } = farm
  const isPromotedFarm = farm.token.symbol === 'WDNERO'
  ...
  const aprTooltip = useTooltip(
    <>
      <Text>
        {t(
          'Global APR calculated using the total amount of active & staked liquidity with the pool WDNERO reward emissions.',
        )}
      </Text>
      <br />
