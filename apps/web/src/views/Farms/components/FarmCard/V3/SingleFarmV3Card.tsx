import { IPendingWDneroByTokenId, PositionDetails } from '@dneroswap/farms'
...
import { Pool } from '@dneroswap/v3-sdk'
...
import { BWDneroV3CardView } from '../../YieldBooster/components/bWDneroV3/CardView'
import {
...
} from '../../YieldBooster/hooks/bWDneroV3/useBWDneroV3Info'
import { useBoostStatus } from '../../YieldBooster/hooks/bWDneroV3/useBoostStatus'
...
interface SingleFarmV3CardProps {
  ...
  token: Token
  quoteToken: Token
  pendingWDneroByTokenIds: IPendingWDneroByTokenId
  onDismiss?: () => void
...
> = ({
  ...
  const { updatedUserMultiplierBeforeBoosted } = useVeWDneroUserMultiplierBeforeBoosted()
  ...
  const totalEarnings = useMemo(
    () => +formatBigInt(pendingWDneroByTokenIds[position.tokenId.toString()] || 0n, 4),
    [pendingWDneroByTokenIds, position.tokenId],
  )

  const earningsBusd = useMemo(() => {
    return new BigNumber(totalEarnings).times(wdneroPrice.toString()).toNumber()
                    ...
                    >
                      {outOfRangeUnstaked ? (
                        <>
                          ...
                          <b> {t('NOT')} </b>
                          {t('earn WDNERO rewards from farm.')}
                        </>
                      ) : (
          ...
          <>
            ...
            <RowBetween flexDirection="column" alignItems="flex-start" flex={1} width="100%">
              <FarmV3HarvestAction
                ...
                disabled={!pendingWDneroByTokenIds?.[position.tokenId.toString()] ?? true}
                userDataReady
