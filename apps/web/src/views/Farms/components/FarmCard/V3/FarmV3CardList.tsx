...
import { AutoRenewIcon, AutoRow, Box, Button, Flex, PreTitle, Text } from '@dneroswap/uikit'
...
const FarmV3CardList: React.FunctionComponent<React.PropsWithChildren<FarmV3CardListProps>> = ({
  ...
  const { stakedPositions, unstakedPositions, lpSymbol, token, quoteToken, pendingWDneroByTokenIds, multiplier } = farm
              ...
              .map((position) => (
                <>
                  <SingleFarmV3Card
                    ...
                    pendingWDneroByTokenIds={pendingWDneroByTokenIds}
                    onDismiss={onDismiss}
                  />
                </>
              ))}
