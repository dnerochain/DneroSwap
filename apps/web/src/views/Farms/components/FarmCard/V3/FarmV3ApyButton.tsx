...
import { Position, encodeSqrtRatioX96 } from '@dneroswap/v3-sdk'
import { FarmWidget } from '@dneroswap/widgets-internal'
import { RoiCalculatorModalV2, useRoi } from '@dneroswap/widgets-internal/roi'
...
import { USER_ESTIMATED_MULTIPLIER, useUserPositionInfo } from '../../YieldBooster/hooks/bWDneroV3/useBWDneroV3Info'
import { BoostStatus, useBoostStatus } from '../../YieldBooster/hooks/bWDneroV3/useBoostStatus'
...
function FarmV3ApyButton_({ farm, existingPosition, isPositionStaked, tokenId }: FarmV3ApyButtonProps) {
  ...
  const wdneroAprFactor = useMemo(
    () =>
      new BigNumber(farm.poolWeight)
        .times(farmV3.wdneroPerSecond)
        .times(365 * 60 * 60 * 24)
        .times(wdneroPrice)
        .div(
    ...
    [
      ...
      farm.poolWeight,
      farmV3.wdneroPerSecond,
      isPositionStaked,
    ],
  )

  const positionWDneroApr = useMemo(
    () =>
      existingPosition
        ? outOfRange
          ? 0
          : new BigNumber(existingPosition.liquidity.toString())
              .times(wdneroAprFactor)
              .div(depositUsdAsBN ?? 0)
              .toNumber()
        : 0,
    [wdneroAprFactor, depositUsdAsBN, existingPosition, outOfRange],
  )
  ...
  const wdneroApr = +(farm.wdneroApr ?? 0)

  const displayApr = getDisplayApr(wdneroApr, lpApr)
  const wdneroAprDisplay = wdneroApr.toFixed(2)
  const positionWDneroAprDisplay = positionWDneroApr.toFixed(2)
  ...
  const estimatedAPR = useMemo(() => {
    return (parseFloat(wdneroAprDisplay) * USER_ESTIMATED_MULTIPLIER + parseFloat(lpAprDisplay)).toLocaleString('en-US', {
      maximumFractionDigits: 2,
    })
  }, [wdneroAprDisplay, lpAprDisplay])
  ...
  const positionDisplayApr = getDisplayApr(+positionWDneroApr, lpApr)
  const positionBoostedDisplayApr = getDisplayApr(boostMultiplier * positionWDneroApr, lpApr)
    ...
    <>
      ...
      <ul>
        <li>
          {t('Farm APR')}:{' '}
          <b>
            {canBoosted && <>{parseFloat(wdneroAprDisplay) * USER_ESTIMATED_MULTIPLIER}% </>}
            <Text
      ...
      <br />
      <Text>
        {t('Calculated using the total active liquidity staked versus the WDNERO reward emissions for the farm.')}
      </Text>
      {canBoosted && (
        <Text mt="15px">
          {t('bWDNERO only boosts Farm APR. Actual boost multiplier is subject to farm and pool conditions.')}
        </Text>
      )}
    ...
    <>
      ...
      <ul>
        <li>
          {t('Farm APR')}:{' '}
          <b>
            {isBoosted && <>{(positionWDneroApr * boostMultiplier).toFixed(2)}% </>}
            <Text
              display="inline-block"
              bold={!isBoosted}
              style={{ textDecoration: isBoosted ? 'line-through' : 'none' }}
            >
              {positionWDneroAprDisplay}%
            </Text>
          </b>
        </li>
        <li>
    ...
    <>
          ...
          <ApyLabelContainer alignItems="center" style={{ textDecoration: 'initial' }} onClick={roiModal.onOpen}>
            {outOfRange ? (
              <TooltipText decorationColor="failure" color="failure" fontSize="14px">
                {positionWDneroApr.toLocaleString('en-US', { maximumFractionDigits: 2 })}%
              </TooltipText>
            ) : (
              <>
      ...
      {wdneroPrice && wdneroAprFactor && (
        <RoiCalculatorModalV2
          ...
          wdneroPrice={wdneroPrice.toFixed(3)}
          wdneroAprFactor={wdneroAprFactor.times(isBoosted ? boostMultiplier : 1)}
          prices={prices}
