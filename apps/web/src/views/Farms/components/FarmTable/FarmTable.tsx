import { RowType } from '@dneroswap/uikit'
import { BIG_ZERO } from '@dneroswap/utils/bigNumber'
import { formatBigInt, getBalanceNumber } from '@dneroswap/utils/formatBalance'
import latinise from '@dneroswap/utils/latinise'
...
const FarmTable: React.FC<React.PropsWithChildren<ITableProps>> = ({ farms, wdneroPrice, userDataReady, header }) => {
  ...
  const generateRow = useCallback(
    (farm: V2StakeValueAndV3Farm): RowProps => {
      ...
      if (farm.version === 2) {
        const row: RowProps = {
          ...
          multiplier: {
            multiplier: farm.multiplier ?? '',
            farmWDneroPerSecond: farmV2Multiplier.getFarmWDneroPerSecond(farm.poolWeight ?? BIG_ZERO),
      ...
      return {
        ...
        multiplier: {
          multiplier: farm.multiplier,
          farmWDneroPerSecond: farmV3Multiplier.getFarmWDneroPerSecond(farm.poolWeight),
        ...
        earned: {
          earnings: +formatBigInt(
            Object.values(farm.pendingWDneroByTokenIds).reduce((total, vault) => total + vault, 0n),
            4,
          ),
          pid: farm.pid,
        },
    ...
    [query.search, farmV3Multiplier, wdneroPrice, farmV2Multiplier, chainId],
  )
