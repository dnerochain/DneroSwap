...
import { Skeleton, useToast, useModal } from '@dneroswap/uikit'
...
export const ProxyHarvestActionContainer = ({ children, ...props }) => {
  ...
  const { onReward, onDone, proxyWDneroBalance } = useProxyStakedActions(props.pid, lpContract)

  return children({ ...props, onReward, proxyWDneroBalance, onDone })
}
...
export const HarvestAction: React.FunctionComponent<React.PropsWithChildren<HarvestActionProps>> = ({
  ...
  return (
    <FarmTableHarvestAction
      ...
      proxyWDneroBalance={proxyWDneroBalance}
