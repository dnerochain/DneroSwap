...
import { useBWDneroProxyContract, useERC20 } from 'hooks/useContract'
import { useEffect, useMemo, useState } from 'react'
import { styled } from 'styled-components'
import { getFullDisplayBalance } from '@dneroswap/utils/formatBalance'
import useAccountActiveChain from 'hooks/useAccountActiveChain'
import { useBWDneroProxyContractAddress } from '../hooks/useBWDneroProxyContractAddress'
...
export const InfoIconBox = styled.div`
  ...
  align-items: center;
`
interface BWDneroMigrateModalProps {
...
export const BWDneroMigrateModal: React.FC<BWDneroMigrateModalProps> = ({
  lpContract,
  stakedBalance,
  onDismiss,
  onUnStack,
  ...
  useEffect(() => {
    if (!bWDneroProxy) return
    bWDneroProxy.read.lpApproved([lpContract.address]).then((enabled) => {
      setIsApproved(enabled)
    })
  }, [lpContract, bWDneroProxy])
