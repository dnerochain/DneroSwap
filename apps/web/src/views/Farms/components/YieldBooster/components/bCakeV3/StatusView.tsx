...
import { Box, Flex, HelpIcon, Text, useMatchBreakpoints, useTooltip } from '@dneroswap/uikit'
import { useMemo } from 'react'
import { useAccount } from 'wagmi'
import { useBWDneroBoostLimitAndLockInfo } from '../../hooks/bWDneroV3/useBWDneroV3Info'
import { BoostStatus } from '../../hooks/bWDneroV3/useBoostStatus'
...
const BoosterTooltip = () => {
  ...
  return (
    <>
      {t(
        `Boost multiplier is calculated based on the staking conditions from both Farms and veWDNERO. Numbers will be automatically updated upon user actions.`,
      )}
    </>
  )
}
...
export const StatusView: React.FC<{
  ...
  const bWDneroMessage = useBWDneroMessage(
    account,
...
const useBWDneroMessage = (
  ...
  const bWDneroMessage = useMemo(() => {
    ...
    if (!locked) return t('Get veWDNERO to activate yield booster')
    if (shouldUpdate) return t('Click to update and increase your boosts.')
    if (isLockEnd) return t('Renew your WDNERO staking to activate yield booster')
