import { useBWDneroFarmBoosterContract, useMasterchef } from 'hooks/useContract'
...
async function getPublicMultiplier({ farmBoosterContract }): Promise<number> {
  const [cAResult, caPercisionResult, boostPercisionResult] = await publicClient({ chainId: ChainId.DNERO }).multicall({
    contracts: [
      {
...
async function getUserMultiplier({ farmBoosterContract, account, pid }): Promise<number> {
  const [multiplierResult, boostPrecisionResult] = await publicClient({ chainId: ChainId.DNERO }).multicall({
    contracts: [
      {
        address: farmBoosterContract.address,
        abi: bWDneroFarmBoosterABI,
