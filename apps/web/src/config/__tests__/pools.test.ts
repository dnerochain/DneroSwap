...
describe.concurrent(
  ...
  () => {
    for (const chainId of SUPPORTED_CHAIN_IDS) {
      ...
      // Pool 0 is special (wdnero pool)
      // Pool 78 is a broken pool, not used, and break the tests
      const idsToRemove = chainId === ChainId.DNERO ? [0, 78] : chainId === ChainId.DNERO_TESTNET ? [0] : []
      ...
      it.each<SerializedPool>(poolsToTest.filter((pool) => pool.earningToken.symbol !== 'DTOKEN'))(
      ...
      it.each<SerializedPool>(poolsToTest.filter((pool) => pool.stakingToken.symbol !== 'DTOKEN'))(
