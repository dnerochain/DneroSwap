import { Currency, CurrencyAmount, Price, TradeType } from '@dneroswap/sdk'
import { ChainId } from '@dneroswap/chains'
import { WDNERO, STABLE_COIN } from '@dneroswap/tokens'
...
export function useStablecoinPrice(
  ...
  const isWDnero = chainId && currency && WDNERO[chainId] && currency.wrapped.equals(WDNERO[chainId])

  const isStableCoin = currency && stableCoin && currency.wrapped.equals(stableCoin)

  const shouldEnabled = currency && stableCoin && enabled && currentChainId === chainId && !isWDnero && !isStableCoin
  ...
  const price = useMemo(() => {
    ...
    if (isWDnero && wdneroPrice) {
      return new Price(
        currency,
        stableCoin,
        1 * 10 ** currency.decimals,
        getFullDecimalMultiplier(stableCoin.decimals).times(wdneroPrice.toFixed(stableCoin.decimals)).toString(),
      )
    }
