import { Currency, WNATIVE } from '@dneroswap/sdk'
...
export default function useWrapCallback(
  ...
  const wdtokenContract = useWNativeContract()
  ...
  return useMemo(() => {
    if (!wdtokenContract || !chainId || !inputCurrency || !outputCurrency) return NOT_APPLICABLE
      ...
      return {
        ...
        execute:
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  const txReceipt = await callWithGasPrice(wdtokenContract, 'deposit', undefined, {
      ...
      return {
        ...
        execute:
          sufficientBalance && inputAmount
            ? async () => {
                try {
                  const txReceipt = await callWithGasPrice(wdtokenContract, 'withdraw', [inputAmount.quotient])
                  const amount = inputAmount.toSignificant(6)
                  const wrap = WNATIVE[chainId].symbol
  ...
  }, [wdtokenContract, chainId, inputCurrency, outputCurrency, t, inputAmount, balance, addTransaction, callWithGasPrice])
}
