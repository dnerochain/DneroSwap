...
import { CurrencyAmount, Price, Currency } from '@dneroswap/sdk'
...
export default function useGasOverhead(
  ...
  const inputIsDTOKEN = inputAmount?.currency.symbol === 'DTOKEN'

  const gasCostInInputTokens = useTradeExactIn(requiredGasAsCurrencyAmount, inputIsDTOKEN ? null : inputAmount?.currency)

  const bufferedOutputAmount = useMemo(() => {
    if (inputIsDTOKEN) return requiredGasAsCurrencyAmount
  ...
  }, [gasCostInInputTokens, requiredGasAsCurrencyAmount, inputIsDTOKEN])
  ...
  const realExecutionPrice = useMemo(() => {
    if (!inputAmount || (!gasCostInInputTokens && !inputIsDTOKEN) || !realInputAmount || !outputAmount) return null

    if (inputIsDTOKEN && requiredGasAsCurrencyAmount.greaterThan(inputAmount.asFraction)) return undefined
  ...
  }, [realInputAmount, outputAmount, inputAmount, gasCostInInputTokens, inputIsDTOKEN, requiredGasAsCurrencyAmount])
