import { Currency, CurrencyAmount, Fraction, Percent, Price, Token } from '@dneroswap/sdk'
import { ChainId } from '@dneroswap/chains'
import { isActiveV3Farm } from '@dneroswap/farms'
import {
  AtomBox,
  AutoColumn,
  AutoRow,
...
import { ConfirmationModalContent, NextLinkFromReactRouter } from '@dneroswap/widgets-internal'

import { MasterChefV3, NonfungiblePositionManager, Pool, Position, isPoolTickInRange } from '@dneroswap/v3-sdk'
    ...
    inRange && ownsNFT && hasActiveFarm && !isStakedInMCv3 && !hasMerkl ? (
      <Message variant="primary" mb="2em">
        <Box>
          ...
          <Text display="inline">
            {t(
              'has an active DneroSwap farm. Stake your position in the farm to start earning with the indicated APR with WDNERO farming.',
            )}
          </Text>
  ...
  if (isMobile) {
    return (
      <Box>
        <AutoRow>
          <ScanLink
            useDneroCoinFallback={chainId ? ChainLinkSupportChains.includes(chainId) : false}
