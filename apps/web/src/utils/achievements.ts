...
import { getIfoConfig } from '@dneroswap/ifos'
...
export const getClaimableIfoData = async (account: string, t: TranslateFunction): Promise<Achievement[]> => {
  const ifosList = (await getIfoConfig(ChainId.DNERO)) || []
  const ifoCampaigns = ifosList.filter((ifoItem) => ifoItem.campaignId !== undefined)

  const dneroClient = publicClient({ chainId: ChainId.DNERO })

  // Returns the claim status of every IFO with a campaign ID
  const claimStatusesResults = await dneroClient.multicall({
  ...
  const claimableIfoDataResult = (await dneroClient.multicall({
    contracts: calls,
    allowFailure: false,
