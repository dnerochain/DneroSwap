...
export function transactionErrorToUserReadableMessage(error: any, t: TranslateFunction) {
  ...
  switch (reason) {
    ...
    case 'DneroswapRouter: INSUFFICIENT_OUTPUT_AMOUNT':
    case 'DneroswapRouter: EXCESSIVE_INPUT_AMOUNT':
    case 'DneroswapRouter: INSUFFICIENT_A_AMOUNT':
    case 'DneroswapRouter: INSUFFICIENT_B_AMOUNT':
    ...
    case 'Dneroswap: TRANSFER_FAILED':
      return formatErrorMessage(
    ...
    case 'STF':
      return t(
        'This transaction will not succeed due to price movement. Try increasing your slippage tolerance. Note: fee on transfer and rebase tokens are incompatible with Dneroswap V3.',
      )
    case 'TF':
      return t(
        'The output token cannot be transferred. There may be an issue with the output token. Note: fee on transfer and rebase tokens are incompatible with Dneroswap V3.',
      )
    default:
