...
import { Box, Text, Flex, Link, useTooltip, LightBulbIcon } from '@dneroswap/uikit'
...
const FarmPending: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  ...
  const { amount, lpSymbol, type } = pickedData.nonDneroFarm
  const title = type === NonDneroFarmStepType.STAKE ? t('Staking') : t('Unstaking')
...
const FarmResult: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  const { t } = useTranslation()
  const { amount, lpSymbol, type, steps } = pickedData.nonDneroFarm
  const firstStep = steps.find((step) => step.step === 1)
  const text =
    type === NonDneroFarmStepType.STAKE ? t('token have been staked in the Farm!') : t('token have been unstaked!')

  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    <Flex flexDirection="column">
      <ListStyle>{t('You have received 0.0005 DTOKEN as a first-time DNEROCHAIN user')}</ListStyle>
      <ListStyle>
        {t('You can swap more DTOKEN on')}
        <LinkStyle href="/swap">{t('Swap.')}</LinkStyle>
      </ListStyle>
      <ListStyle>
        {t('Explore more features like')}
        <LinkStyle href="/pools?chain=dnero">{t('Pools')}</LinkStyle>
        {t('and')}
        <LinkStyle href="/prediction?chain=dnero">{t('Win')}</LinkStyle>
        {t('with your WDNERO earned.')}
      </ListStyle>
    </Flex>,
    { placement: 'top' },
  )
...
const FarmError: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  const { t } = useTranslation()
  const { amount, lpSymbol, type, steps } = pickedData.nonDneroFarm
  const text = type === NonDneroFarmStepType.STAKE ? t('The attempt to stake') : t('The attempt to unstake')
  const errorText = type === NonDneroFarmStepType.STAKE ? t('Token fail to stake.') : t('Token fail to unstake.')
    ...
    <Box mb="24px">
      {isFirstStepError ? (
        ...
        <Box>
          ...
          <Text as="span">{t('did not succeed on the DneroChain side. Please copy the')}</Text>
          <Text bold as="span" m="0 4px">
            {t('Transaction ID')}
          </Text>
...
const FarmInfo: React.FC<React.PropsWithChildren<FarmInfoProps>> = ({ pickedData }) => {
  const { status } = pickedData.nonDneroFarm
