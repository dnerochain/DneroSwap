...
import { Token, Currency } from '@dneroswap/sdk'
...
import { ListLogo } from '@dneroswap/widgets-internal'
              ...
              {!!token.chainId && (
                <>
                  ...
                  <Link href={getBlockExploreLink(token.address, 'address', token.chainId)} external>
                    ...
                    {token.chainId === ChainId.DNERO && <DneroScanIcon color="primary" ml="4px" />})
                  </Link>
                </>
              )}
            </Grid>
            {token && SUPPORT_ONLY_DNERO.includes(token.chainId) && (
              <Flex mt={['20px', '20px', '0']}>
