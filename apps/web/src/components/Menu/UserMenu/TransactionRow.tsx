import { BlockIcon, CheckmarkCircleIcon, Flex, DneroScanIcon, RefreshIcon } from '@dneroswap/uikit'
...
const renderIcon = (txn: TransactionDetails) => {
  const { receipt, nonDneroFarm } = txn
  if (!txn.receipt || nonDneroFarm?.status === FarmTransactionStatus.PENDING) {
    return <RefreshIcon spin width="24px" />
  }

  const isFarmStatusSuccess = nonDneroFarm ? nonDneroFarm.status === FarmTransactionStatus.SUCCESS : true
...
const TransactionRow: React.FC<React.PropsWithChildren<TransactionRowProps>> = ({ txn, chainId, type, onDismiss }) => {
  ...
  return (
    <TxnLink onClick={onClickTransaction}>
      ...
      <TxnIcon>
        <DneroScanIcon width="24px" color="primary" />
      </TxnIcon>
    </TxnLink>
  )
}
