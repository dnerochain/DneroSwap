import { Button, Modal, Text, Grid, Box, Message, MessageText } from '@dneroswap/uikit'
  ...
  return (
    <Modal title={title || t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="360px">
        <Text bold>{t('It’s a DNEROCHAIN only feature')}</Text>
        ...
        <Text small>
          {t(
            'Our Pools, Limit, Trading Competition, Prediction, Lottery and NFTs features are currently available only on DneroChain! Come over and join the community in the fun!',
          )}
        </Text>
        {canSwitch ? (
          <Button
            ...
            isLoading={isLoading}
            onClick={() => (isWrongNetwork ? switchNetworkLocal(ChainId.DNERO) : switchNetworkAsync(ChainId.DNERO))}
          >
            {t('Switch to %chain%', { chain: 'DNEROCHAIN' })}
          </Button>
        ) : (
