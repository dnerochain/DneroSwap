...
export const NetworkModal = ({ pageSupportedChains = SUPPORT_ONLY_DNERO }: { pageSupportedChains?: number[] }) => {
  ...
  const isDTOKENOnlyPage = useMemo(() => {
    return pageSupportedChains?.length === 1 && pageSupportedChains[0] === ChainId.DNERO
  }, [pageSupportedChains])
  ...
  if (isPageNotSupported && isDTOKENOnlyPage) {
    return (
