import { Button, Grid, Message, MessageText, Modal, Text } from '@dneroswap/uikit'
...
export function UnsupportedNetworkModal({ pageSupportedChains }: { pageSupportedChains: number[] }) {
  ...
  const chainId = useLocalNetworkChain() || ChainId.DNERO
      ...
      <Grid style={{ gap: '16px' }} maxWidth="336px">
          ...
          <Button
            ...
            onClick={() => {
              if (supportedMainnetChains.map((c) => c.id).includes(chainId)) {
                ...
                switchNetworkAsync(ChainId.DNERO)
              }
            }}
          >
        ...
        {isConnected && (
          <Button
            ...
            onClick={() =>
              logout().then(() => {
                switchNetworkLocal(ChainId.DNERO)
              })
            }
          >
            {t('Disconnect Wallet')}
