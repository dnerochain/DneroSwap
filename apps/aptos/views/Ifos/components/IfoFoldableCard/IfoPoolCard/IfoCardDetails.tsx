...
const MaxTokenEntry = ({ maxToken, ifo }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyWDnero = true
  const isV3 = ifo.version >= 3
  const { t } = useTranslation()

  const basicTooltipContent = t(
    'For the public sale, each eligible participant will be able to commit any amount of WDNERO up to the maximum commit limit, which is published along with the IFO voting proposal.',
  )
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].wdneroToBurn !== '$0' && <FooterEntry label={t('WDNERO to burn:')} value={ifo[poolId].wdneroToBurn} />}
          <FooterEntry
