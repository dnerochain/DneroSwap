...
import type { DeserializedFarm } from '@dneroswap/farms'
import { FarmWithStakedValue, filterFarmsByQuery } from '@dneroswap/farms'
  ...
  const farmsList = useCallback(
    (farmsToDisplay: DeserializedFarm[]): FarmWithStakedValue[] => {
      const farmsToDisplayWithAPR: FarmWithStakedValue[] = farmsToDisplay?.map((farm) => {
        ...
        const { wdneroRewardsApr } = isActive
          ? getFarmApr(new BigNumber(farm.poolWeight ?? 0), wdneroPrice, totalLiquidity, regularWDneroPerBlock ?? 0)
          : { wdneroRewardsApr: 0 }
    ...
    [query, isActive, wdneroPrice, regularWDneroPerBlock, lpRewardsAprs],
  )
