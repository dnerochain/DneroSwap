...
const FarmTable: React.FC<React.PropsWithChildren<ITableProps>> = ({
  ...
  const generateRow = (farm) => {
    ...
    const farmWDneroPerSecond =
      farm.poolWeight && wdneroPerBlock ? (Number(farm.poolWeight) * Number(wdneroPerBlock)) / 1e8 : 10
    ...
    const row: RowProps = {
      ...
      multiplier: {
        multiplier: farm.multiplier,
        rewardWDneroPerSecond: true,
        farmWDneroPerSecond:
          farmWDneroPerSecond === 0
            ? '0'
            : farmWDneroPerSecond < 0.000001
            ? '<0.000001'
            : `~${farmWDneroPerSecond.toFixed(6)}`,
        totalMultipliers,
      },
