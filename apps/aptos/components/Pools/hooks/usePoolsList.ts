import { useAccountResources, useTableItem } from '@dneroswap/awgmi'
...
import { Coin, Pair, PAIR_LP_TYPE_TAG } from '@dneroswap/aptos-swap-sdk'
import { WDNERO_PID } from 'config/constants'
import { useInterval, useLastUpdated } from '@dneroswap/hooks'

import { PoolResource } from '../types'
import transformWDneroPool from '../transformers/transformWDneroPool'
...
export const usePoolsList = () => {
  ...
  // const tranformWDneroPool = useWDneroPool({ balances, chainId })
  ...
  return useMemo(() => {
    ...
    // const wdneroPool = tranformWDneroPool()

    // return wdneroPool ? [wdneroPool, ...syrupPools] : syrupPools
...
export const useWDneroPool = ({ balances, chainId }) => {
  ...
  const wdneroFarm = useMemo(() => getFarmConfig(chainId)?.find((f) => f.pid === WDNERO_PID), [chainId])

  const wdneroToken = useMemo(() => wdneroFarm?.token && deserializeToken(wdneroFarm?.token), [wdneroFarm])
  ...
  const pairs: [Coin, Coin][] = useMemo(() => {
    ...
    return [
      [aptCoin, usdcCoin],
      [aptCoin, wdneroToken],
      [usdcCoin, wdneroToken],
    ]
  }, [aptCoin, wdneroToken, usdcCoin])
  ...
  }, [wdneroToken, pairsWithInfo, usdcCoin])
  ...
  return useCallback(() => {
    if (!masterChef || !wdneroFarm) return undefined
    const wdneroPoolInfo = masterChef.data.pool_info[WDNERO_PID]

    return transformWDneroPool({
      balances,
      wdneroPoolInfo,
      userInfo,
  ...
  }, [masterChef, wdneroFarm, balances, userInfo, chainId, earningTokenPrice, getNow])
}
