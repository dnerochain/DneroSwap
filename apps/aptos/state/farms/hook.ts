...
import { ChainId, Coin, Pair, PAIR_RESERVE_TYPE_TAG } from '@dneroswap/aptos-swap-sdk'
import { DeserializedFarmsState, deserializeFarm } from '@dneroswap/farms'
import { useAccount, useAccountResource, useCoins, useQueries, useQuery } from '@dneroswap/awgmi'
import {
  FetchCoinResult,
  unwrapTypeArgFromString,
  fetchTableItem,
  FetchAccountResourceResult,
} from '@dneroswap/awgmi/core'
import { getFarmsPrices } from '@dneroswap/farms/farmPrices'
import { BIG_TWO, BIG_ZERO } from '@dneroswap/utils/bigNumber'
...
import { calcPendingRewardWDnero, calcRewardWDneroPerShare } from './utils/pendingWDnero'
...
export const useFarms = () => {
  ...
  const lpInfo = useMemo(() => {
    return farmConfig
      .filter((f) => f.pid !== 0 && f.pid !== WDNERO_PID)
      .concat()
      .map((config) => {
        ...
        const isAptWDneroLp = config.pid === null && config.lpSymbol === 'APT-WDNERO LP'
  ...
  const showWDneroPerSecond = masterChef?.data && new BigNumber(currentDate).lte(masterChef.data.end_timestamp)
  const regularWDneroPerSeconds = showWDneroPerSecond
    ? new BigNumber(masterChef?.data?.wdnero_per_second)
        .times(masterChef.data.wdnero_rate_to_regular)
        .div(new BigNumber(masterChef.data.wdnero_rate_to_regular).plus(masterChef.data.wdnero_rate_to_special))
        .toNumber()
    : 0

  const totalRegularAllocPoint = masterChef?.data.total_regular_alloc_point
  const wdneroPerBlock = masterChef?.data.wdnero_per_second

  return useMemo(() => {
    return {
      userDataLoaded: true,
      poolLength,
      regularWDneroPerBlock: regularWDneroPerSeconds,
      ...
      data: farmsWithPrices
        ...
        .map(deserializeFarm)
        .map((f) => {
          const accWDneroPerShare =
            masterChef?.data && f.pid ? calcRewardWDneroPerShare(masterChef.data, String(f.pid), getNow) : 0
          const earningToken = calcPendingRewardWDnero(
