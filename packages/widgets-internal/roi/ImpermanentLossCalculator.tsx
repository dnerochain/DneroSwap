...
import { Currency, CurrencyAmount, ONE_HUNDRED_PERCENT, ZERO_PERCENT } from "@dneroswap/sdk";
import { FeeCalculator, encodeSqrtRatioX96 } from "@dneroswap/v3-sdk";
import { styled } from "styled-components";
import { WDNERO } from "@dneroswap/tokens";
import { Box, Row, AutoColumn, Toggle, RowBetween, Message } from "@dneroswap/uikit";
...
interface Props {
  ...
  isFarm?: boolean;
  wdneroPrice?: string;
  setEditWDneroPrice: (wdneroPrice: number) => void;
}

const getWDneroAssetsByReward = (chainId: number, wdneroRewardAmount = 0, wdneroPrice: string) => {
  return {
    currency: WDNERO[chainId as keyof typeof WDNERO],
    amount: wdneroRewardAmount,
    price: wdneroPrice,
    value: Number.isFinite(wdneroRewardAmount) ? +wdneroRewardAmount * +wdneroPrice : Infinity,
    key: "WDNERO_ASSET_BY_APY",
  };
};
...
export const ImpermanentLossCalculator = memo(function ImpermanentLossCalculator({
  ...
  const wdneroRewardAmount = useMemo(
    () => (Number.isFinite(wdneroReward) ? +wdneroReward / +wdneroPrice : Infinity),
    [wdneroReward, wdneroPrice]
  );
  ...
  const exitAssets = useMemo<Asset[] | undefined>(() => {
    if (assets && isFarm && currencyA && currencyA.chainId in WDNERO && wdneroPrice) {
      const wdneroPriceToUse =
        assets.find((a) => a.currency.equals(WDNERO[currencyA.chainId as keyof typeof WDNERO]))?.price ?? wdneroPrice;
      return [...assets, getWDneroAssetsByReward(currencyA.chainId, wdneroRewardAmount, wdneroPriceToUse)];
    }
    return assets;
  }, [assets, wdneroRewardAmount, wdneroPrice, currencyA, isFarm]);
  ...
  const getPriceAdjustedAssets = useCallback(
    (newAssets?: Asset[]) => {
      ...
      const [assetA, assetB, maybeAssetWDnero] = newAssets;
      ...
      if (maybeAssetWDnero) {
        adjusted = [
          ...
          {
            ...maybeAssetWDnero,
            ...getWDneroAssetsByReward(assetCurrencyA.chainId, wdneroRewardAmount, String(maybeAssetWDnero.price)),
          },
        ];

        setEditWDneroPrice(+maybeAssetWDnero.price);
      }

      return adjusted;
    },
    // setEditWDneroPrice is not a dependency because it's setState
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [amountA, amountB, tickLower, tickUpper, sqrtRatioX96, wdneroRewardAmount, liquidity]
  );
