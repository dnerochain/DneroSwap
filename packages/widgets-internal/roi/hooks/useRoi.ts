import { Currency, CurrencyAmount, Fraction, ONE, Percent, ZERO } from "@dneroswap/sdk";
import { FeeAmount, FeeCalculator } from "@dneroswap/v3-sdk";
import { formatFraction, parseNumberToFraction } from "@dneroswap/utils/formatFractions";
...
export function useRoi({
  ...
  const {
    apr: wdneroAprInPercent,
    apy: wdneroApy,
    reward: originalWDneroReward,
  } = useRate({
    interest: (wdneroApr && principal && ((wdneroApr / 100) * principal) / 365) ?? 0,
    principal,
    compoundEvery,
    compoundOn,
    stakeFor,
  ...
  const {
    rate: wdneroRate,
    reward: wdneroReward,
    apr: editWDneroAprInPercent,
    apy: editWDneroApy,
  } = useRate({
    interest: (editWDneroApr && principal && ((editWDneroApr / 100) * principal) / 365) ?? 0,
    principal,
    compoundEvery,
    compoundOn,
    stakeFor,
  ...
  return {
    ...
    apr,
    apy,
    wdneroApr: wdneroAprInPercent,
    editWDneroApr: editWDneroAprInPercent,
    wdneroApy,
    editWDneroApy,
    wdneroRate,
    wdneroReward,
    originalWDneroReward,
  };
}
