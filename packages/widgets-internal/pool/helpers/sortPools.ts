...
import { getWDneroVaultEarnings } from "./getWDneroVaultEarnings";
      ...
      return orderBy(
        ...
        (pool: DeserializedPool<T>) => {
          ...
          if (pool.vaultKey) {
            ...
            if (pool.stakingTokenPrice && vault?.totalWDneroInVault?.isFinite()) {
              totalStaked =
                +formatUnits(BigInt(vault.totalWDneroInVault.toString()), pool?.stakingToken?.decimals) *
