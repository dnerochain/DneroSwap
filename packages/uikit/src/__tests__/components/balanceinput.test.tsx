...
it("renders correctly with unit prop", () => {
  const { asFragment } = renderWithProvider(
    <BalanceInput value="14" currencyValue="15 USD" unit="WDNERO" onUserInput={handleChange} />
  );
  expect(asFragment()).toMatchInlineSnapshot(`
    <DocumentFragment>
      .c6 {
...
it("renders correctly with unit prop and switchEditingUnits", () => {
  ...
  expect(asFragment()).toMatchInlineSnapshot(`
    ...
    .c10:active:not(:disabled):not(.dneroswap-button--disabled):not(.dneroswap-button--disabled) {
      opacity: 0.85;
    ...
    @media (hover: hover) {
      .c10:hover:not(:disabled):not(.dneroswap-button--disabled):not(.dneroswap-button--disabled):not(:active) {
        opacity: 0.65;
      }
    }
