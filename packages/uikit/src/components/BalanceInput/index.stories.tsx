...
export const UnitDisplay: React.FC<React.PropsWithChildren> = () => {
  const WDNERO_PRICE = 69;
  const [wdneroValue, setWDneroValue] = useState("1006.086956");

  const wdneroToUSD = (input: string) => {
    const convertedToUSD = parseFloat(input) * WDNERO_PRICE;
  ...
  const handleWDneroChange = (input: string) => {
    setWDneroValue(input);
  };
  ...
  return (
    <>
      <Box width="300px" mb="24px">
        <BalanceInput
          onUserInput={handleWDneroChange}
          value={wdneroValue}
          currencyValue={wdneroToUSD(wdneroValue)}
          placeholder="0.0"
          unit="WDNERO"
        />
      </Box>
...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const WDNERO_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"WDNERO" | "USD">("WDNERO");
  const conversionUnit = editingUnit === "WDNERO" ? "USD" : "WDNERO";
  const [values, setValues] = useState({
    WDNERO: "1006.086957",
    USD: `${1006.086957 * WDNERO_PRICE}`,
  });
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "WDNERO" ? "USD" : "WDNERO";
  ...
  const handleWDneroChange = (input: string) => {
    ...
    if (editingUnit === "WDNERO") {
      setValues({
        WDNERO: input,
        USD: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat * WDNERO_PRICE}`,
      });
    } else {
      setValues({
        WDNERO: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / WDNERO_PRICE}`,
        USD: input,
      });
    }
  };
