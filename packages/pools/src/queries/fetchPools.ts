...
import { DNERO_BLOCK_TIME, getPoolsConfig } from '../constants'
...
async function fetchUpgradedPoolsTimeLimits(
  ...
  return pools.map((wdneroPoolConfig, index) => {
    const [startTimestamp, endTimestamp] = startEndResult[index]
    return {
      sousId: wdneroPoolConfig.sousId,
...
const fetchLegacyPoolsBlockLimits = async (
  ...
  const getTimestampFromBlock = (targetBlock: number) => {
    return Number(block.timestamp) + (targetBlock - Number(block.number)) * DNERO_BLOCK_TIME
  }
  return pools.map((wdneroPoolConfig, index) => {
...
export const fetchPoolsStakingLimitsByBlock = async ({
  ...
  const validPools = poolsConfig
    .filter(isLegacyPool)
    .filter((p) => p.stakingToken.symbol !== 'DTOKEN' && !p.isFinished)
  ...
  return fromPairs(
    poolStakingChunkedResultRaw.map((stakingLimitRaw, index) => {
      ...
      const numberSecondsForUserLimit = numberBlocksForUserLimit * DNERO_BLOCK_TIME
...
export const fetchPoolsProfileRequirement = async (
  ...
  const poolProfileRequireCalls = livePoolsWithV3
    .map(({ contractAddress }) => {
      return (['dneroswapProfileIsRequested', 'dneroswapProfileThresholdPoints'] as const).map(
        (method) =>
          ({
            abi: sousChefV3ABI,
            address: contractAddress,
