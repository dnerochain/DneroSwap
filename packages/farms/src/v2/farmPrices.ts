import { BIG_ONE, BIG_TWO, BIG_ZERO } from '@dneroswap/utils/bigNumber'
...
// either via direct calculation if farm is X-DTOKEN or X-BUSD
// or via quoteTokenFarm which is quoteToken-DTOKEN or quoteToken-BUSD farm
export const getFarmBaseTokenPrice = (
  ...
  // We can only calculate profits without a quoteTokenFarm for BUSD/DTOKEN farms
  if (!quoteTokenFarm) {
    return BIG_ZERO
  }
  ...
  // If the farm's quote token isn't BUSD or WDTOKEN, we then use the quote token, of the original farm's quote token
  // i.e. for farm PNT - pBTC we use the pBTC farm's quote token - DTOKEN, (pBTC - DTOKEN)
  // from the DTOKEN - pBTC price, we can calculate the PNT - BUSD price
  ...
  // Catch in case token does not have immediate or once-removed BUSD/WDTOKEN quoteToken
  return BIG_ZERO
}
...
const filterFarmsByQuoteToken = (
  farms: SerializedFarmPublicData[],
  preferredQuoteTokens: string[] | undefined = ['BUSD', 'WDTOKEN'],
): SerializedFarmPublicData => {
