...
const masterChefV2Abi = [
  {
    ...
    name: 'poolInfo',
    outputs: [
      { internalType: 'uint256', name: 'accWDneroPerShare', type: 'uint256' },
...
export const fetchMasterChefData = async (
  ...
  try {
    ...
    const chainId = isTestnet ? ChainId.DNERO_TESTNET : ChainId.DNERO
...
export const fetchMasterChefV2Data = async ({
  ...
  try {
    const chainId = isTestnet ? ChainId.DNERO_TESTNET : ChainId.DNERO
    const [poolLength, totalRegularAllocPoint, totalSpecialAllocPoint, wdneroPerBlock] = await provider({
      chainId,
    }).multicall({
      contracts: [
        {
