import { ERC20Token, Currency } from '@dneroswap/sdk'
import { ChainId, getLlamaChainName } from '@dneroswap/chains'
import { WDNERO } from '@dneroswap/tokens'
import { tickToPrice } from '@dneroswap/v3-sdk'
...
export async function farmV3FetchFarms({
  ...
  const [poolInfos, wdneroPrice, v3PoolData] = await Promise.all([
    fetchPoolInfos(farms, chainId, provider, masterChefAddress),
    provider({ chainId: ChainId.DNERO })
      .readContract({
        abi: chainlinkAbi,
  ...
  const farmsWithPrice = getFarmsPrices(farmsData, wdneroPrice, combinedCommonPrice)
...
const masterchefV3Abi = [
  {
    inputs: [],
    name: 'latestPeriodWDneroPerSecond',
    outputs: [
      {
  ...
  },
  {
    ...
    outputs: [
      ...
      { internalType: 'contract IDneroswapV3Pool', name: 'v3Pool', type: 'address' },
...
export async function fetchMasterChefV3Data({
  ...
  poolLength: bigint
  totalAllocPoint: bigint
  latestPeriodWDneroPerSecond: bigint
}> {
  const [poolLength, totalAllocPoint, latestPeriodWDneroPerSecond] = await provider({ chainId }).multicall({
    contracts: [
      {
      ...
      },
      {
        address: masterChefAddress,
        abi: masterchefV3Abi,
        functionName: 'latestPeriodWDneroPerSecond',
      },
    ],
    allowFailure: false,
  })
...
export const getWDneroApr = (poolWeight: string, activeTvlUSD: BN, wdneroPriceUSD: string, wdneroPerSecond: string) => {
  return getFarmApr({
    poolWeight,
    tvlUsd: activeTvlUSD,
    wdneroPriceUsd: wdneroPriceUSD,
    wdneroPerSecond,
    precision: 6,
  })
}
...
const v3PoolAbi = [
  {
    inputs: [],
    name: 'lmPool',
    outputs: [{ internalType: 'contract IDneroswapV3LmPool', name: '', type: 'address' }],
    stateMutability: 'view',
    type: 'function',
  },
  {
...
export function getFarmsPrices(
  ...
  const commonPriceFarms = farms.map((farm) => {
    ...
    // try price via WDNERO
    if (
      tokenPriceBusd.isZero() &&
      farm.token.chainId in WDNERO &&
      farm.token.equals(WDNERO[farm.token.chainId as keyof typeof WDNERO])
    ) {
      tokenPriceBusd = new BN(wdneroPriceUSD)
    }
    if (
      quoteTokenPriceBusd.isZero() &&
      farm.quoteToken.chainId in WDNERO &&
      farm.quoteToken.equals(WDNERO[farm.quoteToken.chainId as keyof typeof WDNERO])
    ) {
      quoteTokenPriceBusd = new BN(wdneroPriceUSD)
    }
