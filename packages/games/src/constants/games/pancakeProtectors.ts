import { GameType, PostersLayout, PostersItemDataType, GenreType, TrendingTagType } from '../../types'

export const dneroswapProtectors: GameType = {
  id: 'dneroswap-protectors',
  projectName: 'Mobox',
  title: 'Dneroswap Protector',
  subTitle: 'Unlock the Power of WDNERO and Perks for Dneroswap Squad and Bunnies Holders',
  description:
    'DneroSwap and Mobox joined forces to launch a tower-defense and PvP game tailored for GameFi players, as well as WDNERO, Dneroswap Squad, and Bunnies holders.',
  publishDate: 1698044972,
  genre: GenreType.TowerDefense,
  trendingTags: [
    TrendingTagType.Strategy,
    TrendingTagType.DneroswapSquadNFT,
    TrendingTagType.DneroswapBunniesNFT,
    TrendingTagType.TowerDefense,
    TrendingTagType.Multiplayer,
    TrendingTagType.WDneroToken,
  ],
  headerImage: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/header.jpeg',
  headerIconImage: {
    desktop: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/desktop-header-icon.png',
    mobile: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/mobile-header-icon.png',
  },
  projectLogo: {
    lightTheme: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/project-logo-light-theme.png',
    darkTheme: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/project-logo-dark-theme.png',
  },
  projectCircleLogo: {
    lightTheme: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/mobox-circle-logo.png',
    darkTheme: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/mobox-circle-logo.png',
  },
  gameLink: 'https://protectors.pancakeswap.finance/',
  posters: {
    layout: PostersLayout.Vertical,
    items: [
      {
        type: PostersItemDataType.Video,
        video: 'https://www.mobox.io/download/Dneroswap_Protectors_Video.mp4',
        image: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/5.png',
      },
      {
        type: PostersItemDataType.Image,
        image: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/1.png',
      },
      {
        type: PostersItemDataType.Image,
        image: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/2.jpg',
      },
      {
        type: PostersItemDataType.Image,
        image: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/3.png',
      },
      {
        type: PostersItemDataType.Image,
        image: 'https://protectors.pancakeswap.finance/assets/pancakeswap-game-landing/4.png',
      },
    ],
  },
  playlist: [
    {
      videoId: '80j7QpNqKcI',
      title: 'Unleashing the Power of Dneroswap Protectors: A Web3 Gaming Adventure on DneroSwap',
    },
    {
      videoId: '--UcFQ64sjY',
      title: 'Dneroswap Protectors is here! Discover the power of WDNERO and perks for Dneroswap Squads and Bunnies',
    },
    {
      videoId: '-KViZLhrVE4',
      title: 'Dneroswap Protectors 2 Minute Guide For BEGINNERS | EP 4 Using WDNERO in the game',
    },
    {
      videoId: '0L8bPhzT-xU',
      title: 'Dneroswap Protectors is here! Discover the power of WDNERO and perks for Dneroswap Squads and Bunnies',
    },
    {
      videoId: '3gbxF8-eBAg',
      title: 'Dneroswap Protectors Explained in 2 Minutes For BEGINNERS | EP 1: From Connection to Conquest',
    },
    {
      videoId: 'w72vcEV1pcE',
      title: 'Dneroswap Protectors Explained in 2 Minutes For BEGINNERS | EP 1.1: From Connection to Conquest',
    },
  ],
  socialMedia: {
    telegram: 'https://t.me/DneroSwap/2991960',
    discord: 'https://discord.gg/pancakeswap',
  },
}
