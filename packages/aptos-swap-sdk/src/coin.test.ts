...
describe('Coin', () => {
  ...
  const ADDRESS_DTOKEN = '0x8c805723ebc0a7fc5b7d3e7b75d567918e806b3461cb9fa21941a9edc0220bf::devnet_coins::DevnetDTOKEN'

  describe('#equals', () => {
    it('fails if address differs', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'WDNERO').equals(new Coin(ChainId.TESTNET, ADDRESS_TWO, 18, 'WDNERO'))
      ).toBe(false)
    })

    it('false if chain id differs', () => {
      expect(
        new Coin(ChainId.MAINNET, ADDRESS_ONE, 18, 'WDNERO').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'WDNERO'))
      ).toBe(false)
    })

    it('true if only decimals differs', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 9, 'WDNERO').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'WDNERO'))
      ).toBe(true)
    })

    it('true if address is the same', () => {
      expect(
        new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'WDNERO').equals(new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'WDNERO'))
      ).toBe(true)
    })

    it('true on reference equality', () => {
      const token = new Coin(ChainId.TESTNET, ADDRESS_ONE, 18, 'WDNERO')
    ...
    it('sortBefore', () => {
      ...
      const dtoken = new Coin(ChainId.TESTNET, ADDRESS_DTOKEN, 8, 'DTOKEN')
      expect(dtoken.sortsBefore(busd)).toBe(true)

      expect(AptosCoin.onChain(ChainId.TESTNET).sortsBefore(tokenA)).toBe(true)

      expect(
        new Coin(ChainId.TESTNET, '0x16::test_coins::TestDTOKEN', 8, '').sortsBefore(
          new Coin(ChainId.TESTNET, '0x123123123::swap_utils::TestDTOKEN', 8, '')
        )
      ).toBe(false)

      expect(
        new Coin(ChainId.TESTNET, '0x16::dneroswap::WDNERO', 8, '').sortsBefore(
