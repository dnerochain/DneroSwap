...
import { iWDneroABI } from '../abis/IWDnero'
import { ifoV7ABI } from '../abis/IfoV7'
import { IWDNERO } from '../constants/contracts'
...
): GetContractReturnType<typeof iWDneroABI, PublicClient, WalletClient> => {
  const address = getContractAddress(IWDNERO, chainId)
  if (!address || address === '0x') {
    throw new Error(`IWDNERO not supported on chain ${chainId}`)
  }
  const publicClient = provider({ chainId })

  // @ts-ignore
  return getContract({ abi: iWDneroABI, address, publicClient, walletClient })
}
