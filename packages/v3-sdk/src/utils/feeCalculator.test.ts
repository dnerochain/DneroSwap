import { BigintIsh, ZERO, CurrencyAmount, Fraction, Percent } from '@dneroswap/swap-sdk-core'
...
describe('#getLiquidityBySingleAmount', () => {
  it('input with token 0 amount', () => {
    const amount = getLiquidityBySingleAmount({
      amount: CurrencyAmount.fromRawAmount(dneroTokens.usdt, '100'),
      currency: dneroTokens.busd,
      tickLower: -953,
      tickUpper: 953,
  ...
  it('input with token 1 amount', () => {
    const amount = getLiquidityBySingleAmount({
      amount: CurrencyAmount.fromRawAmount(dneroTokens.busd, '200'),
      currency: dneroTokens.usdt,
      tickLower: -953,
      tickUpper: 953,
      sqrtRatioX96: encodeSqrtRatioX96(1, 1),
    })
...
describe('#getDependentAmount', () => {
  it('input with token 0 amount', () => {
    ...
    expect(amount.currency).toEqual(dneroTokens.busd)
  })

  it('input with token 1 amount', () => {
    const amount = getDependentAmount({
      amount: CurrencyAmount.fromRawAmount(dneroTokens.busd, '100'),
      currency: dneroTokens.usdt,
      tickLower: -1000,
      tickUpper: 1000,
      sqrtRatioX96: encodeSqrtRatioX96(1, 1),
    ...
    expect(amount.quotient).toEqual(99n)
    expect(amount.currency).toEqual(dneroTokens.usdt)
  })
})
