...
import { wrapCorsHeader, handleCors, CORS_ALLOW } from '@dneroswap/worker-utils'
import BigNumber from 'bignumber.js'
import { fetchWDneroPrice, saveFarms, saveLPsAPR } from './handler'
...
router.get('/price/wdnero', async (_, event) => {
  const cache = caches.default
  const cacheResponse = await cache.match(event.request)
  let response
  if (!cacheResponse) {
    const price = await fetchWDneroPrice()
    response = json(
